...
function M.encode(code)
  return vim.fn.trim(vim.fn.system("echo " .. vim.fn.shellescape(code) .. " | base64"))
end
...
function M.start()
  ...
  session_context.terminal_buffer_id = vim.api.nvim_get_current_buf()
end
...
vim.keymap.set({ "n", "v" }, "<leader>om", M.start, { silent = true, noremap = true })
