...
- **Catalog** (`00`–`13`) — how the system is _laid out_: exhaustive coverage of every one of the 1278 files in the effective git file set, named or grouped by exact chezmoi source path. Use it to drill from a concept to the precise file.
...
5. [`03b-agent-skills-hooks.mmd`](03b-agent-skills-hooks.mmd) — every file under `exact_dot_agents/` (52 skills + 7 hooks + references).
...
12. [`07c-bin-commands.mmd`](07c-bin-commands.mmd) — every thin command in `exact_bin/` (73) grouped by purpose + deployed command/shared internals in `home/exact_lib/` (41 command/shared library files).
...
16. [`11-scripts-helpers.mmd`](11-scripts-helpers.mmd) — every file in `scripts/` (77): shared parsers, MCP/model generators, AI KB, tests.
