...
- **Catalog** (`00`–`13`) — how the system is _laid out_: exhaustive coverage of every one of the 1531 files in the effective git file set, named or grouped by exact chezmoi source path. Use it to drill from a concept to the precise file.
...
- [`S3-flow-pickers-handoff.mmd`](S3-flow-pickers-handoff.mmd) — flow: owner-scoped tmux picker handoff pins and successor popup relaunch.
...
5. [`03b-agent-skills-hooks.mmd`](03b-agent-skills-hooks.mmd) — every file under `exact_dot_agents/` (58 skill entrypoints + hooks, references, and assets).
6. [`05-tmux-pickers.mmd`](05-tmux-pickers.mmd) — tmux pickers, owner-scoped handoff pins, and popup lifecycle.
...
10. [`07c-bin-commands.mmd`](07c-bin-commands.mmd) — every thin command in `exact_bin/` (85) grouped by purpose + deployed command/shared internals in `home/exact_lib/` (117 command/shared library files), spanning 40 command libraries plus shared helpers.
...
14. [`11-scripts-helpers.mmd`](11-scripts-helpers.mmd) — every file in `scripts/` (141): shared parsers, reconcilers, MCP/model/mirror generators, AI KB, session/cache diagnostics, artifact ledger, tests.
15. [`12-ai-tool-configs.mmd`](12-ai-tool-configs.mmd) — every per-tool AI config (Cursor, Claude, Codex, Antigravity, OpenCode, Pi, tuicr, lgtm).
...
The per-subtree file-census counts (`Every file under X (N)` and the total effective-file count) are enforced by `make verify-mermaids` ([`scripts/verify_mermaids.py`](../scripts/verify_mermaids.py), part of `make check`): it recomputes each count from the effective git file set (tracked files that still exist plus untracked, non-ignored files) and fails on drift, so a count change must update both the diagram prose and the script's census table.
