...
- **Catalog** (`00`–`13`) — how the system is _laid out_: exhaustive coverage of every one of the 1351 files in the effective git file set, named or grouped by exact chezmoi source path. Use it to drill from a concept to the precise file.
...
10. [`07c-bin-commands.mmd`](07c-bin-commands.mmd) — every thin command in `exact_bin/` (82) grouped by purpose + deployed command/shared internals in `home/exact_lib/` (75 command/shared library files), spanning 33 command libraries plus shared helpers.
...
14. [`11-scripts-helpers.mmd`](11-scripts-helpers.mmd) — every file in `scripts/` (112): shared parsers, reconcilers, MCP/model/mirror generators, AI KB, session/cache diagnostics, artifact ledger, tests.
