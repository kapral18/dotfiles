...
- **Catalog** (`00`–`13`) — how the system is _laid out_: exhaustive coverage of every one of the 1525 files in the effective git file set, named or grouped by exact chezmoi source path. Use it to drill from a concept to the precise file.
...
10. [`07c-bin-commands.mmd`](07c-bin-commands.mmd) — every thin command in `exact_bin/` (85) grouped by purpose + deployed command/shared internals in `home/exact_lib/` (114 command/shared library files), spanning 40 command libraries plus shared helpers.
