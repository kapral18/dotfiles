#!/usr/bin/env bash
# Stage GH-picker selection so the popup loop can hand off to ,palantir summon.
# Usage: handoff_to_palantir_pin.sh SELECTION_FILE PIN_FILE
set -euo pipefail
...
context_builder="$script_dir/../github/lib/gh_palantir_handoff_context.py"
