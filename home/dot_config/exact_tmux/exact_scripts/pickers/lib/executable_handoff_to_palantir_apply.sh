#!/usr/bin/env bash
# Consume a palantir handoff pin and prompt for a goal, then summon a legion.
# Usage: handoff_to_palantir_apply.sh PIN_FILE
set -euo pipefail
...
# The palantir context sibling lives inside the handoff namespace, which gh_popup
...
# that copy, so the deferred `,palantir summon` legion can still read it. Fail closed
...
tmux command-prompt \
  ...
  "run-shell -b \"cd $ws_arg && ,palantir summon \\\"%1\\\"\""
