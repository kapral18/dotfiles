...
# GH <-> session pivots share one private pin/sentinel space and can never
# read, clear, or consume another popup loop's state. The owner removes the namespace on EXIT.
...
# Every exit ends (removes) this namespace by owner pid.
_gh_popup_cleanup() {
