...
local function extract_path_from_item(item, base_dir)
  if item.filename and item.filename ~= "" then
    return common_utils.normalize_path(item.filename, base_dir)
  end
  ...
  then
    ...
    if name and name ~= "" then
      return common_utils.normalize_path(name, base_dir)
    end
  end
  ...
  if item.text and item.text ~= "" then
    ...
    if candidate then
      return common_utils.normalize_path(candidate, base_dir)
    end
  end
...
function M.copy_qf_paths_to_clipboard()
  ...
  if #paths > 0 then
    common_utils.copy_to_clipboard(table.concat(paths, "\n"))
  end
end
