...
complete -c ,mcp-token -l retry-connect-timeouts -d "With --bridge, retry one exact upstream connection-timeout response"
complete -c ,mcp-token -l no-proactive-rotation -d "With --login, skip proactive rotation but ensure current-workspace auth (seed from a verified cached chain, browser last); critical/expired/revoked still block"
