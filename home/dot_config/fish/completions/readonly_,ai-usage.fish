...
complete -c ',ai-usage' -l harness -d 'Restrict to a harness (repeatable)' -r -a 'claude codex omp opencode pi'
complete -c ',ai-usage' -l by -d 'Group rows' -r -a 'session harness model agent'
...
complete -c ',ai-usage' -l cost -d 'Add 1-hour cache writes and an Anthropic list-price equivalent'
