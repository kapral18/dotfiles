...
function show_help() {
  cat << EOF
Usage: ,tmux-run-all [--all] <pattern> [exclude_pattern] <command>
...
Flags:
    --all           Run command in all idle panes (default: first idle pane of first window)
    -h, --help      Show this help message

Examples:
    ,tmux-run-all "work-*" "git status"
    ,tmux-run-all "dev-*" "*test*" "npm run build"
    ,tmux-run-all "*" "main" "echo 'hello world'"
    ,tmux-run-all --all "work-*" "git status"

Notes:
    - Uses shell pattern matching (*, ?, [])
    - If exclude_pattern is omitted, no sessions are excluded
    - Default: Command is executed in first idle pane (prioritizes first window)
    - With --all: Command is executed in all idle panes of each session
...
function get_first_idle_pane_in_session() {
  local session="$1"

  mapfile -t panes_list < <(tmux list-panes -t "$session" -F '#{pane_id}	#{session_name}:#{window_index}.#{pane_index}	#{pane_current_command}' 2> /dev/null || true)

  if [[ ${#panes_list[@]} -eq 0 ]]; then
    return 1
  fi

  local trimmed=()
  for entry in "${panes_list[@]}"; do
    trimmed+=("${entry%%$'\r'}")
  done
  panes_list=("${trimmed[@]}")

  # First pass: look for idle pane in first window (window 0)
  for pane_info in "${panes_list[@]}"; do
  ...
  # Second pass: look for idle pane in any window
  for pane_info in "${panes_list[@]}"; do
    pane_command="${pane_info#*$'\t'}"
    pane_command="${pane_command#*$'\t'}"
...
function run_command_in_session() {
  local session="$1"
  local command="$2"
  local all_panes="${3:-false}"

  if [[ "$all_panes" == "true" ]]; then
    # Original behavior: run in all idle panes
    ...
    log_info "Running command in idle panes of session '$session'"
  else
    # New behavior: run only in first idle pane (prefer first window)
    if ! first_pane_info=$(get_first_idle_pane_in_session "$session"); then
      printf '0 0 1\n'
      log_info "No idle pane found in session '$session' (skipped)"
      return 0
    fi

    session_panes=("$first_pane_info")
    log_info "Running command in first idle pane of session '$session'"
  fi
  ...
  if [[ "$all_panes" == "true" ]]; then
    # Original behavior: handle all panes with prioritization
    ...
    # New behavior: run only in the single first window pane (already validated as idle)
    for pane_info in "${session_panes[@]}"; do
      pane_id="${pane_info%%$'\t'*}"
      rest="${pane_info#*$'\t'}"
      pane_display="${rest%%$'\t'*}"
...
function main() {
  ...
  # Shift arguments based on offset
  local remaining_args=("${args[@]:$arg_offset}")
  ...
  case ${#remaining_args[@]} in
    ...
    3)
      pattern="${remaining_args[0]}"
      exclude_pattern="${remaining_args[1]}"
      command="${remaining_args[2]}"
      ;;
    *)
  ...
  for session in "${matching_sessions[@]}"; do
    if output=$(run_command_in_session "$session" "$command" "$all_panes"); then
      status=0
    else
      status=$?
