...
Artifacts and runtime state live under XDG_CACHE_HOME/agent-artifacts. The
current worktree/cwd is used only as identity metadata, never as a write target.
"""
...
def detect_ambient_theme(root: Path | None = None) -> dict[str, Any]:
    ...
    if (project_root / ".mermaids").is_dir() and (project_root / "home").is_dir():
        name = "dotfiles"
        label = "Dotfiles terminal"
        markers.extend([".mermaids/", "home/"])
        tokens = {
            "bg": "#0f172a",
            "surface": "#111827",
            "surface_2": "#1e293b",
    ...
    elif (project_root / "docusaurus.config.js").exists() or (project_root / "website").is_dir():
        name = "docs"
        label = "Documentation"
        markers.extend(["docusaurus.config.js or website/"])
        tokens = {
            "bg": "#f8fafc",
            "surface": "#ffffff",
            "surface_2": "#eef2ff",
    ...
    elif package_keys.intersection({"react", "vue", "svelte", "next", "vite", "@vitejs/plugin-react"}):
        name = "web-app"
        label = "Web app"
        markers.append("package.json frontend dependencies")
        tokens = {
            "bg": "#f6f7fb",
            "surface": "#ffffff",
            "surface_2": "#f1f5f9",
    ...
    elif any((project_root / marker).exists() for marker in ("go.mod", "Cargo.toml", "pyproject.toml")):
        name = "codebase"
        label = "Codebase"
        markers.extend(
...
:root {{
  --aa-bg: {tokens["bg"]};
  --aa-surface: {tokens["surface"]};
  --aa-surface-2: {tokens["surface_2"]};
  --aa-ink: {tokens["ink"]};
  --aa-muted: {tokens["muted"]};
  --aa-line: {tokens["line"]};
  --aa-accent: {tokens["accent"]};
  --aa-accent-2: {tokens["accent_2"]};
  --aa-code-bg: {tokens["code_bg"]};
}}
:where(body) {{
  min-height: 100vh;
  margin: 0;
  ...
  color: var(--aa-ink);
  background:
    radial-gradient(circle at 20% 0%, color-mix(in srgb, var(--aa-accent) 14%, transparent), transparent 30rem),
    radial-gradient(circle at 90% 10%, color-mix(in srgb, var(--aa-accent-2) 10%, transparent), transparent 26rem),
    var(--aa-bg);
}}
:where(h1, h2, h3) {{
  margin: 0 0 0.75rem;
...
:where(main, .artifact-page, .artifact-shell) {{
  max-width: 1080px;
  margin: 0 auto;
  padding: clamp(34px, 6vw, 68px) clamp(18px, 4vw, 36px) 128px;
}}
:where(.card, .panel, .callout, section[data-card]) {{
  background: color-mix(in srgb, var(--aa-surface) 90%, transparent);
...
:where(.density-compact) {{
  font-size: 0.92rem;
  line-height: 1.42;
}}
:where(.density-compact .card, .density-compact .panel, .density-compact .callout, .density-compact section[data-card]) {{
  border-radius: 14px;
  padding: 13px;
}}
:where(.checklist) {{
...
:where(.checklist li) {{
  ...
  align-items: start;
  padding: 9px 11px;
  background: color-mix(in srgb, var(--aa-surface-2) 66%, transparent);
  border: 1px solid var(--aa-line);
  border-radius: 12px;
}}
:where(.checklist li)::before {{
  content: "□";
  color: var(--aa-accent);
  font-weight: 900;
}}
:where(.checklist li[data-state="done"])::before {{
  content: "✓";
...
:where(th) {{
  background: color-mix(in srgb, var(--aa-surface-2) 86%, transparent);
  color: var(--aa-muted);
  font-size: 0.78rem;
...
:where(.pill, .metric) {{
  ...
  border: 1px solid var(--aa-line);
  border-radius: 999px;
  background: color-mix(in srgb, var(--aa-surface-2) 72%, transparent);
  padding: 5px 9px;
}}
</style>
"""
...
<title>{safe}</title>
<style>
  body {{ margin: 0; font: 16px/1.5 system-ui, sans-serif; background: #111827; color: #f9fafb; }}
...
<main>
  <h1>{safe}</h1>
  <p>Replace this cached artifact with the content you want reviewed.</p>
  <p>This file lives under <code>~/.cache/agent-artifacts</code>, not the worktree.</p>
</main>
</body>
</html>
"""
...
def start_server(sdir: Path) -> str:
    ...
    with log.open("ab") as log_fh:
        child = subprocess.Popen(
            [sys.executable, str(Path(__file__).resolve()), "server", "--session-dir", str(sdir)],
...
(() => {
  ...
  root.innerHTML = `
    <style>
      ...
      .head { padding: 10px 12px 8px 14px; }
      ...
      button:hover { background: rgba(148, 163, 184, 0.3); }
      .primary {
      ...
      .item strong { color: #f8fafc; font-size: 11px; }
      .item span { color: #94a3b8; font-size: 11px; }
      .hint { color: #94a3b8; font-size: 11px; margin-top: 5px; }
    </style>
    <div class="box" id="${HOVER_ID}" hidden></div>
    <div class="box" id="${SELECTED_ID}" hidden></div>
    <section class="dock" aria-label="Agent live feedback overlay">
      <div class="head">
        <div class="context" id="context"><strong>Live overlay armed.</strong><span>Click to pin. Alt-click expands. Pause to use the app normally.</span></div>
        <div>
      ...
      <div class="body">
        <textarea id="prompt" placeholder="Write feedback for the pinned live element. Cmd+Enter adds to tray."></textarea>
        <div class="tray" id="tray" aria-label="Queued feedback items"></div>
        <div class="actions">
          <span class="hint" id="status">Capture is on; page clicks are intercepted until paused.</span>
          <div>
  ...
  window.__agentArtifactLiveOverlay = {
    destroy,
    pause: () => { paused = true; updatePaused(); },
    resume: () => { paused = false; updatePaused(); },
    drain: () => pendingBatches.splice(0, pendingBatches.length),
  };
})();
"""
    return script.replace("__NAME__", json.dumps(name)).replace("__BASE_URL__", json.dumps(base_url))
...
<meta charset="utf-8">
<meta name="viewport" content="width=device-width, initial-scale=1">
<title>{safe_name} · Agent Artifact</title>
<style>
  ...
  * {{ box-sizing: border-box; }}
  ...
  .dock.expanded {{
    ...
    box-shadow: 0 26px 72px rgba(2, 6, 23, 0.46);
  }}
  .dock-head {{
    display: flex;
    align-items: center;
...
<section class="dock" id="dock" aria-label="Agent feedback dock">
  ...
  <div class="dock-body">
    <div class="anchor-card" id="anchorCard" aria-live="polite">
      <strong id="anchorTitle">Pinned selection</strong>
      <code id="anchorSelector"></code>
      <p id="anchorText"></p>
    </div>
    <textarea id="prompt" placeholder="Write feedback for the current anchor. Cmd+Enter adds to tray."></textarea>
    <div class="tray" id="tray" aria-label="Queued feedback items"></div>
    <div class="actions">
      <div class="chips">
        <button type="button" data-prompt="Make this clearer and more scannable.">Clarify</button>
        <button type="button" data-prompt="Tighten the layout and reduce visual noise.">Tighten</button>
        <button type="button" data-prompt="Turn this into concrete next steps.">Next steps</button>
      </div>
      <div>
        <button id="clear" type="button">Clear</button>
        <button id="add" type="button">Add</button>
        <button id="end" type="button">End</button>
        <button class="primary" id="send" type="button">Send batch</button>
      </div>
    </div>
    <div class="hint" id="status">Click to pin, select text to promote to its parent area, Alt-click repeatedly to expand toward the full page.</div>
  </div>
</section>
<script>
...
function resetAnchorContext() {{
  context = {{}};
  $("context").innerHTML = "<strong>No anchor yet.</strong><span>Click or select content. Alt-click expands the pinned area.</span>";
...
async function sendFeedbackBatch() {{
  ...
  const response = await fetch("/api/feedback/{safe_name}", {{
    method: "POST",
    headers: {{ "content-type": "application/json" }},
    body: JSON.stringify({{ items }})
  }});
