...
`ui-proof` checks the built runtime against the intended UI state/behavior and captures proof to attach to a PR.
...
- generic browser automation with no intended UI state/behavior to check against: `~/.agents/skills/playwriter/SKILL.md` directly
...
- the **intended visual/UI state or behavior** to check against — exactly one of:
  ...
  - a linked issue/design mockup, screenshot, UI behavior repro, or the PR's stated UI goal (`compose-pr`)
