...
The worker loads `~/.agents/skills/k-review/references/judging_core.md` and `~/.agents/skills/k-review/references/judging_pipeline.md` by absolute path from the packet; the root names the applicable gates and MUST NOT read those files to copy their text.
...
pass the named applicable gates and the absolute paths of `~/.agents/skills/k-review/references/judging_core.md` and `~/.agents/skills/k-review/references/judging_pipeline.md`, not their bodies and not a router. Per-harness profile names live in `~/.config/ai/agent-bands.v1.json` → `harnesses.<h>.agents`.
...
Harness-specific invocation caveats in `~/.agents/skills/k-review/references/runtime-harnesses.md` (and, for Pi/OMP, `~/.agents/skills/k-review/references/runtime-harnesses-pi-omp.md`) travel as a packet pointer only; the root does not open them.
