...
- `~/.agents/skills/k-github/SKILL.md` needs issue text before creating/editing an issue
...
2. If the issue body needs contested, historical, product, or team-precedent context not settled by direct references, run Ambient Topic Exploration.
   That workflow lives in `~/.agents/skills/k-review/references/pr_context_audits.md`.
3. Load `~/.agents/skills/k-communication/SKILL.md` before drafting the title/body; a draft-only request is still publication text.
4. Identify the problem statement, expected behavior, actual behavior, and reproduction from verified evidence.
5. Keep repro steps concrete and ordered.
6. Convert local-only observations into portable repro steps.
...
7. If logs/screenshots are referenced, include only what materially helps and redact secrets.
8. If issue creation is in scope, verify `gh issue create --help` support for `--type name`.
   If the target repo exposes GitHub issue types, read the actual type names before choosing one.
9. If the repo belongs to the `elastic` org or is `elastic/kibana`, load `~/.agents/skills/k-elastic-domain/SKILL.md`.
   Apply its issue composition section.
10. Build the issue publication packet.
...
- public issue text must be portable for other maintainers:
  ...
  - strip agentic-session artifacts (SOP references, skill/agent/packet names, `Compatibility impact:` lines, spec paths, ledger references) per `~/.agents/skills/k-communication/SKILL.md` before returning the draft
- GitHub issue type:
...
- When handing the draft to `k-github` for issue creation/editing, include the packet outside the issue body so the GitHub skill can build its publication preflight.
