...
It is a **review-readiness map**, not a review.
The reader should finish it knowing the system, the load-bearing lines, and the order to inspect GitHub.
Concepts introduced by the PR are first-class content, not incidental prose. If a choice doesn't serve that, cut it.
...
- **Introduced concepts inventory:** each business/domain concept or logic layer the PR adds or changes.
  Include source anchors, what/how/why explanation, left-sidebar label, right-sidebar note, and the medium that teaches it fastest.
...
- **GitHub handoff:** close with an ordered path through the real PR.
  Use primary hunks first, then guardrails/tests, then supporting/mechanical files. This is the final product of the presentation.
...
5. **Act II — The load-bearing changes.** Only the **1–3** changes that actually carry risk/insight/security.
   ...
   Each beat should be a _consequence or obligation created by the previous one_ (e.g. "switching to the new transport is what _creates_ the framing problem the next beat solves").
...
- **PRIMARY** — carries the goal or the main risk. Gets a full Act II beat (code or diagram). Cap at **3**.
...
- Fill or resize the concept primer, readiness map, concept sidebar links, story-section links, and notes sidebar before Act I.
  Every concept link must target a concept card, every readiness/story link must target its section, and every note card must be reachable from a sidebar link.
...
- Write for the **target reviewer's level** (e.g. a senior engineer of the relevant system). Use real symbol/API names.
  No dumbed-down analogies.
- For business/domain logic, make the semantics explicit before the mechanics:
  what changed for the user/system, how the PR represents it, and why the reviewer should care.
