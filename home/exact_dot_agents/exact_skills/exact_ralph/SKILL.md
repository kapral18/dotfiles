...
- `planner` — emits a JSON spec (goal, target_artifact ABS, success_criteria, complexity, executor_count, max_iterations, max_minutes, iteration_task_seed). Defaults to `cursor + claude-opus-4-8-thinking-xhigh --mode plan` (read-only planning; cannot edit). On pi, equivalent is `--no-tools`.
- `executor` — does one concrete step toward the spec, ends output with a `LEARNING:` line and `RALPH_DONE`. May emit `RALPH_REPLAN` to force a replan. Defaults to `cursor + composer-2.5 --force` (cursor's purpose-built agentic coder, auto-approves commands). Needs full tools either way.
- `reviewer` — judges the executor's last step against the spec, emits a JSON verdict (`pass|needs_iteration|fail|block`) followed by `RALPH_DONE`. Defaults to `cursor + claude-opus-4-8-thinking-xhigh --mode ask` (read-only Q&A with shell read access for verification probes like `od -c` / `wc -c`). On pi, equivalent is `--no-tools`.
