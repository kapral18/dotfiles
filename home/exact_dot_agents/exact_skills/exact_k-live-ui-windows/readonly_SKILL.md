...
0. `VBoxManage --version` must succeed.
   If VirtualBox itself is not installed, return `Blocked` immediately — installing it is the user's job, not this skill's.
1. Resolve the VM name from an authoritative source only.
...
- Leave the guest OS configuration untouched, including Guest Additions; the one-time debug-port launch inside Windows is the user's manual step, not this skill's.
- Keep an already-running VM's start type unchanged.
- This skill controls the guest only through VirtualBox's own host-side VM/network commands.
