...
Use `,proof` to turn a hard-triggered non-review/non-build freeform task into a small proof ledger without forcing `/spec`, `/build`, or Ralph.
...
- formal `/build` or Ralph runs that already carry a criteria ledger, unless the user asks for a separate freeform proof receipt
- `review`, `light-review`, `/agent-review`, or PR-fix flows unless the user explicitly asks for a separate freeform proof receipt
...
Load `~/.agents/skills/proof/references/cli.md` when you need command syntax, evidence type rules, storage details, JSON shape, or report behavior.
