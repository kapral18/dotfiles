...
- drafting issue/PR text from an already-clear problem: `~/.agents/skills/compose-issue/SKILL.md` / `compose-pr`
- reviewing an existing plan/design document: the `review` skill (plan mode)
...
2. **Close the forks.**
   Run the SOP §3.0 Intent Loop with the interview discipline from `~/.agents/skills/interview-me/SKILL.md`:
