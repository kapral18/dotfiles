...
Reference for the `backport` CLI (`sorenlouv/backport`) as wired into `elastic/kibana`, used by the `k-kbn-backport` flow.
...
The interactive pause is the seam the `k-kbn-backport` flow plugs into: resolve + stage in the checkout, then send ENTER to let the tool finish push + PR creation.
