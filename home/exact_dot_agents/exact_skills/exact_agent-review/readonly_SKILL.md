...
- applying fixes, drafting payloads, or touching GitHub only after the relevant `review`/`communication`/`github`/`git` gates
  - load `communication` before wording any human-visible draft
...
The Base-Branch Context Gate in `~/.agents/skills/review/references/shared_rules.md` is blocking and controller-owned.
...
7. **Act only after judgment.**
   ...
   - Before composing any human-visible text in this step — review summaries, draft comments/suggestions, thread replies, or PR-level feedback — load `~/.agents/skills/communication/SKILL.md` via the Skill tool and word the text to its contract.
