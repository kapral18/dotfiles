...
- Multiline bodies/comments: use bash/zsh `$'...'` so `\n` becomes real newlines;
  `\\n` escapes inside normal quotes with `gh api -f body=...` come through as literal text.
- Test Plan is inferred from the change surface; run the smallest sufficient checks and record commands/results.
- Repro-driven fixes need portable local repro steps plus commands/results; publish only portable evidence, keeping session-specific details such as private hostnames or browser automation state out.
...
- Treat `pending_approval` as an open item; surface it in approval or immediately after creation/readback.
