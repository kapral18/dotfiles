...
Subcommands:
    ...
    patch <dist_package> <versions_dir> <active_version>
        Apply OLD->V2 and V1->V2 migrations to active bundles. Reports per-file
        action taken. Dumps anchor context for unknown-shape files so a human
        or agent can re-derive the regex patterns. Exit 0.
...
def classify(source):
    ...
    if any(a in source for a in PICKER_ANCHORS):
...
def patch_source(source):
    ...
    return new_source, old_count, v1_count
...
def cmd_patch(dist_package, versions_dir, active_version):
    ...
    for p in files:
        ...
        if state_before == "v2-good":
            ...
            continue
        if state_before in ("v1-collapses-variants", "old-empty-picker"):
            new_source, old_count, v1_count = patch_source(source)
            details = []
            if old_count > 0:
            ...
            patched_files += 1
            total_replacements += old_count + v1_count
