---
name: fix-cursor-cli
description: Targeted repair skill for local Cursor CLI regressions (macOS quarantine/native-module policy failures and `/model` empty-list, duplicate-variant, or missing Thinking-suffix picker/footer regressions). Use when those symptoms are present.
...
- the prompt footer briefly shows a thinking model and then drops the `Thinking` suffix while the underlying model slug is unchanged
...
2. Read-only state check (preferred first step on any machine):
   ...
   - Reports picker state per active bundle (`v2-good` / `display-collapses-thinking` / `v1-collapses-variants` / `old-empty-picker` / `unknown-shape`) and exits non-zero if any active bundle needs a fix.
3. Choose a targeted fix:
   ...
   - if evidence is interactive-only (`/model` empty, duplicate-variant entries, or missing `Thinking` suffixes in `/model`/footer while `cursor-agent models` succeeds), run:
...
4. Re-verify end-to-end:
   ...
   - a fresh interactive session launched with a thinking model shows the `Thinking` suffix in the footer
   - `/model` shows entries and distinguishes thinking/non-thinking variants with a `Thinking` suffix
...
6. Share learning:
   - state what symptom was detected, what fix was chosen, and what bundle state the script reported (`v2-good` / `display-collapses-thinking` / `v1-collapses-variants` / `old-empty-picker` / `unknown-shape`)
...
- `scripts/picker_patch.py`: var-name-agnostic regex classifier and migrator for the model-picker bundle. Handles the known states (`old-empty-picker` -> V2, `v1-collapses-variants` -> V2, `display-collapses-thinking` -> display-normalized V2, `v2-good` no-op) across all webpack chunks containing the picker/display code, and dumps anchor context for unknown shapes.
...
- State whether `fix_cursor_cli.sh` changed anything and what worst-state was observed before/after (`v2-good` / `display-collapses-thinking` / `v1-collapses-variants` / `old-empty-picker` / `unknown-shape`).
