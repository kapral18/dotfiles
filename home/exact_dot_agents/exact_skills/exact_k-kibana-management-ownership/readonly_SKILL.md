...
- checking whether changed paths fall under `@elastic/kibana-management` (the user's team) before a side effect, per SOP §3.3 Ownership Gate
...
2. Map each path to its owner with `,codeowners --owner-of <path>`.
   - Use `--owner-of` per path rather than exact-matching changed files against `,codeowners -p kibana-management`;
...
3. Classify:
   ...
   - Any path out-of-team -> list those paths with their owning teams; propose the owning teams as reviewers and flag that the side effect needs explicit approval (SOP §3.3).
   - Unowned paths -> say so explicitly rather than guessing an owner.
...
- CODEOWNERS missing or `,codeowners` unavailable -> skip ownership classification and say so; directory names are not ownership evidence.
