...
Usage:
  ,get-age-buckets --pattern "src/**/*.ts"
  ,get-age-buckets --pattern "*.md,*.mdx"
  ,get-age-buckets --pattern "docs/**/*.asciidoc,*.md"
...
EXAMPLE_USAGE = (
    "Examples:\n  ,get-age-buckets --pattern 'src/**/*.ts'\n  ,get-age-buckets --pattern 'docs/**/*.md' --format json\n"
)


class FriendlyHelpFormatter(argparse.ArgumentDefaultsHelpFormatter, argparse.RawDescriptionHelpFormatter):
    pass
...
def parse_bucket_spec(spec: str) -> List[Bucket]:
    ...
    for part in spec.split(","):
        ...
        if part.startswith(">"):
            ...
            if "-" not in part:
                raise ValueError(f"Invalid bucket fragment '{part}' (expected A-B or >C)")
            a, b = part.split("-", 1)
            start = int(a)
            end = int(b)
            label = f"{start}-{end}d"
...
def compute_last_content_change(patterns: List[str], rename_limit: int | None) -> Dict[str, int]:
...
def format_output(counts: Dict[str, int], buckets: List[Bucket], fmt: str, header: bool) -> str:
    ...
    if fmt == "json":
        return json.dumps(
            {
                "total": total,
                "buckets": [{"bucket": b, "count": c, "percent": round(p, 2)} for b, c, p in rows],
            },
            indent=2,
        )
    if fmt == "tsv":
