---
name: beads-workflow
description: "Local Beads workflow via bdlocal: create/update/close beads, manage dependencies and curated notes, and operate on $BEADS_DIR/.beads/ with explicit user permission for every bead operation. Use when the user mentions beads, bd, bdlocal, BEADS_DIR, or managing tasks/issues in the beads DB. Do NOT use for general task tracking or non-beads work."
---
...
- The user is asking for git or GitHub operations (use the git/gh skills).

Local policy:

- Run all commands through `bdlocal` (invokes `bd --db
...
Epic with external link: `bdlocal create "Auth epic" -t epic -p 1 --external-ref "https://github.com/.../issues/123" --json`
