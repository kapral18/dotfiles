...
description: "GitHub operations using the gh CLI: create/edit PRs and issues, manage labels/assignees/milestones/projects, and use GitHub APIs (REST/GraphQL). Use when the user mentions GitHub, gh, PR/issue creation or editing, or provides a GitHub URL. Any GitHub side effect requires explicit user approval unless the user explicitly instructs otherwise. Do NOT use for draft-only composing or PR review feedback."
---
...
- Resolve the PR number/URL with:
  - `gh prw --number`
  - `gh prw --url`
- If `gh prw` fails, ask for the PR URL (or use `-R OWNER/REPO` if the user
  clearly intends a different repo).

When NOT to use:

- The user wants to draft PR/issue text only (draft-only): use the `github-compose-*` skills.
- The user wants PR review feedback:
  - Initial pass (batch draft): `~/.agents/skills/github-pr-review-start/SKILL.md`
  - Reply to existing threads: `~/.agents/skills/github-pr-review-reply/SKILL.md`
  - One comment at a time: `~/.agents/skills/github-pr-review/SKILL.md`
- The user wants local git operations (status/diff/commit/rebase): `~/.agents/skills/git-workflow/SKILL.md`.
- The user wants worktree management (create/switch/remove worktrees): `~/.agents/skills/w-workflow/SKILL.md`.
...
- Anchoring rules and when to deep-link vs inline comment live in `~/.agents/skills/github-pr-review/SKILL.md`.
...
- Draft PR bodies using:
  - General repos: `~/.agents/skills/github-compose-pr-general/SKILL.md`
  - Elastic/Kibana repos: `~/.agents/skills/github-compose-pr-elastic/SKILL.md`
- Draft issue bodies using:
  - General repos: `~/.agents/skills/github-compose-issue-general/SKILL.md`
  - Elastic/Kibana repos: `~/.agents/skills/github-compose-issue-elastic/SKILL.md`
- Label proposals for Elastic/Kibana (propose-only): `~/.agents/skills/elastic-kibana-labels-propose/SKILL.md`
- Kibana Management ownership hints: `~/.agents/skills/kibana-management-ownership/SKILL.md`
