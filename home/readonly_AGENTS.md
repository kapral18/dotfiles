...
- **Domain allowlists require verification.** Known-bot allowlists live only in verified domain overlays; do not embed repo/org-specific bot logins in this SOP or generic skills. Without a verified overlay, classify bots only from platform evidence such as GitHub `user.type == "Bot"` or a login ending in `[bot]`.
