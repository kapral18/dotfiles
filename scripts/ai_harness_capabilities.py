...
hook-disposition rule targets a harness whose hook_support isn't "blocking".
"""
...
HOOK_SUPPORT_LEVELS = frozenset({"blocking", "advisory", "none"})
...
def check_rule_capability(rule, snapshot: dict[str, "HarnessCapability"]) -> None:
    ...
    if rule.disposition == "hook" and capability.hook_support != "blocking":
