...
GO_OUTPUT_REL = Path("tools/ralph-tui/internal/state/model_mirrors_generated.go")
...
def _load_ralph_defaults(repo_root: Path) -> dict[str, dict[str, str]]:
    roles = _read_json(repo_root / "home/dot_config/ralph/roles.json")["roles"]
    defaults = {}
    for role, spec in roles.items():
        defaults[role] = {
            "family": spec.get("family") or family_of(spec["model"]),
...
def build_static_mirror(repo_root: str | Path) -> dict[str, Any]:
    ...
    mirror = {
        "adapters": {
            ...
            "ralph": {"harnesses": {"cursor": "curated", "pi": "curated"}},
            "ralph-tui": {"harnesses": {"cursor": "recommended", "pi": "recommended"}},
        },
        "defaults": {
            "harnesses": harness_defaults,
            "ralph": _load_ralph_defaults(root),
        },
        "generated": {
            "deterministic": True,
            ...
            "sources": [
                ...
                "home/dot_config/ralph/roles.json",
            ],
        },
        "harnesses": harnesses,
        "kind": MIRROR_KIND,
...
def validate_mirror(mirror: dict[str, Any]) -> None:
    ...
    roles = mirror["defaults"]["ralph"]
    supported_harnesses = set(mirror["adapters"]["ralph"]["harnesses"])
    for role, spec in roles.items():
        harness = spec.get("harness")
        if harness == "command":
            continue
        if harness not in supported_harnesses:
            raise ValueError(f"Ralph {role} default uses unsupported harness {harness!r}")
        model = spec.get("model")
        if model not in mirror["harnesses"][harness]["curated"]["models"]:
            raise ValueError(f"Ralph {role} default model {model!r} is not in {harness} curated models")
    if roles["reviewer"]["family"] == roles["re_reviewer"]["family"]:
        raise ValueError("Ralph reviewer and re_reviewer families must differ")


def render_go_consumer(mirror: dict[str, Any]) -> str:
    validate_mirror(mirror)
    serialized = json.dumps(mirror, ensure_ascii=False, sort_keys=True, separators=(",", ":"))
    lines = [
        ...
        "",
        "const (",
        f'\tgeneratedModelMirrorSchemaVersion = "{SCHEMA_VERSION}"',
        f'\tgeneratedModelMirrorSHA256        = "{hashlib.sha256(serialized.encode()).hexdigest()}"',
        ")",
        "",
    ...
    for harness in ("cursor", "pi"):
        ...
        for model in mirror["harnesses"][harness]["recommended"]["models"]:
            lines.append(f"\t\t{json.dumps(model)},")
        lines.append("\t},")
    lines.extend(["}", "", "var generatedRalphRoleDefaults = map[string]RoleSpec{"])
    roles = ("planner", "executor", "reviewer", "re_reviewer")
    key_width = max(len(f"{json.dumps(role)}:") for role in roles)
    for role in roles:
        spec = mirror["defaults"]["ralph"][role]
        key = f"{json.dumps(role)}:"
        lines.append(
            f"\t{key.ljust(key_width)} {{Harness: {json.dumps(spec['harness'])}, Model: {json.dumps(spec['model'])}}},"
        )
    lines.extend(["}", ""])
    return "\n".join(lines)
...
def _write_outputs(repo_root: Path, output: Path, go_output: Path) -> None:
    ...
    go_output.write_text(render_go_consumer(mirror))


def _verify_outputs(repo_root: Path, output: Path, go_output: Path) -> None:
    expected_json = render_static_mirror(repo_root)
    expected_go = render_go_consumer(build_static_mirror(repo_root))
    errors = []
    if not output.exists() or output.read_text() != expected_json:
        errors.append(str(output))
    if not go_output.exists() or go_output.read_text() != expected_go:
        errors.append(str(go_output))
    if errors:
        raise SystemExit("generated model mirror artifacts are stale: " + ", ".join(errors))
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    for name in ("generate", "verify"):
        ...
        command.add_argument("--go-output", type=Path)

    adapt = subparsers.add_parser("adapt")
    adapt.add_argument("--mirror", type=Path, required=True)
    adapt.add_argument("--consumer", choices=("launcher", "ralph", "ralph-tui"), required=True)
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.command in {"generate", "verify"}:
        ...
        go_output = args.go_output or root / GO_OUTPUT_REL
        if args.command == "generate":
            _write_outputs(root, output, go_output)
        else:
            _verify_outputs(root, output, go_output)
        return 0
