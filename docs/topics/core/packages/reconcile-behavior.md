...
| --------------- | -------------------------------------------------------------------- | ----------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------ |
| Homebrew        | `brew bundle --global --no-upgrade`                                  | Yes, via `brew bundle cleanup --global --force` | assembled Brewfile is authoritative; installs missing only (upgrades owned by `,update`); self-updating casks pinned                 |
| self-updaters   | `,update --only selfupdaters`                                        | No                                              | scans installed Homebrew `auto_updates` casks; app-specific adapters heal stale artifacts after manual reinstall/forced bundle drift |
