...
| Related  | creation-side sibling of `live-ui-review`; shares [`live-ui-runtime.md`](../../../../home/exact_dot_agents/exact_skills/exact_k-agent-review/exact_references/readonly_live-ui-runtime.md); runs inline in `/build` and `k-compose-pr` |
| Boundary | head-only proof capture; not for reviewing others' changes (`k-review`/`/agent-review` own `live-ui-review`)                                                                                                                           |
...
| Boundary | never auto-triggered by `/agent-review`, `/build`, `k-ui-proof`, or `live-ui-review` — load it by hand only on an explicit user request for Windows/VirtualBox coverage |
