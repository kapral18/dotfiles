...
| Loaded by | named in SOP §3.8 as a duty before drafting; `k-compose-pr`, `k-compose-issue`, `k-git` (commit messages), `k-github`, `k-slack`, `k-google-workspace`, and `k-review` load it, including for draft-only output                         |
