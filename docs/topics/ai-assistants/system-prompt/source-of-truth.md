...
The core SOP owns the default response style, and §6 opens with an accessibility contract: the user is dyslexic, so brevity outranks structure and the default shape is the shortest complete essence (verdict line → optional table or anchor list → one new last line). Structure must earn its space by adding scannable information not present elsewhere (add sections, tables, or lists only when they carry information a shorter form lacks; compression cuts words first, not structure that carries new information). In-session replies may borrow STE (ASD-STE100 Simplified Technical English) sentence habits only when they shrink text (one idea per sentence, active voice, delete filler, condition before command); full ASD-STE100 vocabulary, modal, and sentence-cap discipline applies only when the user asks for STE or docs compliance. The remaining subsections are a debloat requirement built on per-class word budgets (direct answer ≤80, comparison/audit ≤120 + one table or anchor list, multi-part investigation ≤200 across skeleton slots), time neutrality (the model has no valid model of elapsed time, effort, or urgency, so it may never justify scope or shortcuts with them), response shape (reach for a density primitive — verdict line, delta table, anchor list, decision block — before prose; emit a 1-line skeleton first for any answer with ≥3 sections; a section may not restate an item already given in an earlier table/list), and a substance floor that protects evidence, precision, real uncertainty, and safety qualifiers from the compression rules. It synthesizes the applicable guidance from [Stop Slop](https://github.com/hardikpandya/stop-slop) and [i-have-adhd](https://github.com/ayghri/i-have-adhd), with optional STE sentence habits from [SimpleEnglish](https://github.com/AminBlg/SimpleEnglish) / ASD-STE100 when they compress without fighting §6 budgets.
