...
Codex is single-vendor (OpenAI only); there is no cross-family split to make here, so `refute` reports `degraded`. Native Codex 0.146.0 catalog-confirms `gpt-5.6-sol`, `gpt-5.6-terra`, `gpt-5.5`, `gpt-5.4`, and `gpt-5.3-codex` (via `codex debug models`), and does not include `gpt-5.2`; the cheap band therefore runs `gpt-5.4` at high effort, and standard and max both run `gpt-5.6-terra` at max effort (user call, 2026-08-05; `max` is a real codex effort level: low/medium/high/xhigh/max/ultra per `codex debug models`). Codex carries effort per profile as `model_reasoning_effort`, so it is pinnable per agent here unlike on Claude Code or Copilot, and the gate can rewrite both fields on `spawn_agent`. Codex exposes no context-tier dial, so every band is short by construction.
...
| standard           | `@task`                                               | `cursor/kimi-k3-high:high` | `cursor/kimi-k3-high:high` |
| max                | `@default`                                            | `cursor/kimi-k3-high:high` | `cursor/kimi-k3-high:high` |
| counter (on `max`) | none — `@advisor` prices to the same id as `@default` | —                          | —                          |

Verified on 17.2.4: a profile carrying `model: "@smol"` runs on `modelRoles.smol`, and an unknown token fails loudly with `Error: No model selected.` rather than falling back. Provider and model are separated by `/`, never `:` — `cursor:` parses as a bogus provider. Like Pi, OMP's `:<level>` suffix is a single thinking dial the runtime maps straight onto `reasoning`, so "high effort, non-thinking" is not expressible here. Both profiles pin their primary roles to Cursor `kimi-k3-high`, use `composer-2.5` for `smol`, and use Cursor `claude-sonnet-5-high` for `vision`.

`modelRoles.advisor` resolves to the same `cursor/kimi-k3-high:high` as `default` on both profiles, so the deliberate pin leaves no second family to refute with: `max` carries no counter, and refutation is `families=same (reduced independence)` (SOP §3.5). The falsification lanes are the one priced exception (user call, 2026-08-05): `adversarial-verifier` and `criteria-verifier` pin concrete `cursor/kimi-k3-max:high` in their profile frontmatter, spending the 2x cursor effort tier only there while every other lane stays on `@default`. The concrete id bypasses `modelRoles` per-profile pricing on purpose — the pin is identical on work and personal. An invariant asserts OMP carries a counter only while `advisor` differs from `default`, so restoring cross-family refutation means repricing the role first, not adding the band back.
