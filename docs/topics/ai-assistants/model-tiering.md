...
| `refute`      | max      | Breaking a conclusion: adversarial verification, criteria refutation, blind clarity           | Delegate, and to a **different model family** than the lane being refuted (`family: counter`)                |
...
Only `refute` is `family: counter`. Same-family reviewers inherit the same blind spots and the same context contamination, so a refuter that shares a family with the work it audits is worth much less. Where a harness cannot field a second family, resolution reports `degraded` rather than presenting the weaker pass as a real cross-family one.
...
1. **Profiles declare it.** [`home/.chezmoitemplates/agent-model.partial`](../../../home/.chezmoitemplates/agent-model.partial) resolves `agent → category → band → model` at apply time, so each agent file under `dot_<harness>/exact_agents/` renders a concrete id. Copilot and Gemini have no per-agent files for their built-ins, so [`scripts/generate_subagent_models.py`](../../../scripts/generate_subagent_models.py) writes `subagents.agents.*` and `agents.overrides.*` from the same registry.
...
| Gemini      | none                                                                  | `invoke_agent` takes no model argument and `BeforeModel` carries no agent identity                                       |
...
| ----------- | ----------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------- |
| Cursor      | Baked into the model-ID suffix (`gpt-5.6-terra-high`, `glm-5.2-high`)                                                                     | `cursor-agent models` live catalog                                                                          |
...
| ------------------ | ------------------- | ------ | -------- | ------- |
| cheap              | `claude-sonnet-4-6` | high   | off      | short   |
| standard           | `claude-opus-5`     | high   | off      | short   |
| max                | `claude-opus-5`     | high   | off      | short   |
| counter (on `max`) | none                | —      | —        | —       |

Claude Code cannot take a cross-vendor counter: it accepts only Claude-family selectors, and an unknown id is not remapped — it reaches the API and returns `API model not found`. `refute` therefore resolves to the primary `max` pick and reports `degraded`. Opus 5 is both the orchestration and the review pick, and standard collapses onto it: user decision, Claude Code only — the cost-driven ban on opus-5 (closed 2026-08-03) is deliberately **not** applied to this harness and still holds for Codex, Cursor, Copilot, and OMP. `alwaysThinkingEnabled: false` keeps Opus non-thinking at high effort. Cheap is the one cost rung and takes Sonnet 4.6; Haiku 4.5 is no longer an active pick, and neither are Fable or GPT-5.6 Sol. `gpt-5.3-codex` is not in the Anthropic-only catalog, so cheap takes a Claude rather than the cross-harness codex-tier pick.

The gate can only clamp Claude to a family alias (`sonnet`, `opus`, `haiku`, `fable`), not to a full id, so it compares alias _rank_ and acts only when the caller asked for something more capable than the band's. An unqualified `Agent` call is left alone, which keeps the profile's more precise id rather than promoting a cheap-band pick to whatever `ANTHROPIC_DEFAULT_SONNET_MODEL` resolves to. Standard and max both project to `opus`, so the alias cannot separate them — the same collision the two Sonnet bands used to have, and the profile frontmatter is what holds each band.
...
Context stays short: bare `claude-opus-5`/`claude-sonnet-4-6` are the short-window selectors, and the `[1m]` suffix is what would swap them onto the 1M window. Nothing in the tier map asks for it, and `,claude-openrouter` passes bare OpenRouter ids for the same reason.
...
Codex is single-vendor (OpenAI only); there is no cross-family split to make here, so `refute` reports `degraded`. Native Codex 0.146.0 catalog-confirms `gpt-5.6-sol`, `gpt-5.6-terra`, `gpt-5.5`, `gpt-5.4`, and `gpt-5.3-codex` (via `codex debug models`), and does not include `gpt-5.2`; the cheap band therefore runs `gpt-5.4` at high effort, standard runs `gpt-5.6-sol` at high effort, and max runs `gpt-5.6-terra` at high effort. Codex carries effort per profile as `model_reasoning_effort`, so it is pinnable per agent here unlike on Claude Code or Copilot, and the gate can rewrite both fields on `spawn_agent`. Codex exposes no context-tier dial, so every band is short by construction.

### Copilot CLI

| Band               | Model               | Effort                                                         |
| ------------------ | ------------------- | -------------------------------------------------------------- |
| cheap              | `gpt-5.3-codex`     | high                                                           |
...
| standard           | `gpt-5.6-sol-xhigh`                      | xhigh  | long    |
| max                | `gpt-5.6-terra-max`                      | max    | long    |
| counter (on `max`) | none — the ban left one family reachable | —      | —       |

Cursor's bands are read by the gate alone, because `cursor-agent` does not discover home-level agent files. That matters for which ids are legal: the `Task` tool takes a far narrower whitelist than the 197 ids `cursor-agent models` lists. Only `claude-fable-5-medium`, `claude-opus-5-high`, `claude-sonnet-5-thinking-max`, `composer-2.5`, `composer-2.5-fast`, `cursor-grok-4.5-high-fast`, `gpt-5.6-sol-xhigh`, and `gpt-5.6-terra-max` resolve for a subagent; anything else fails the spawn with `Invalid model selection`. An invariant pins the bands to that list. glm-5.2 is unusable (closed 2026-08-03), so cheap stays on `composer-2.5`. Policy reserves Terra for the max band: standard uses `gpt-5.6-sol-xhigh` and max uses `gpt-5.6-terra-max`. Gemini 3.6 Flash is sellable to a Cursor main session but is absent from the whitelist, so it is not an option for a subagent here. Both bands name `composer-2.5` rather than `composer-2.5-fast`: Cursor prices Composer 2.5 at $0.5/$2.5 and describes the fast variant as "A faster variant with the same intelligence" at $3/M input and $15/M output, so `-fast` buys speed at 6x, never a cheaper rung. An invariant keeps it out of every band.

The cost-driven ban (closed 2026-08-03) moved the lanes off `claude-opus-5-high`, which is what had made the lane/verifier pairing cross-family. Both review-lane roles are now `gpt-5.6-terra-max`, `max` carries no counter, and refutation is `families=same (degraded)` — report it, never present it as a cross-family pass.

Cursor cannot honor the short-context policy for the GPT-5.6 max band: it publishes the GPT-5.6 ids exclusively as 1M ids (`gpt-5.6-terra-max — GPT-5.6 Terra 1M`), with no short variant to select, so that row records the window it actually gets. The effort/thinking split is a real Cursor id-scheme fact — `claude-opus-5-high` (plain) and `claude-opus-5-thinking-high` are distinct, real model IDs — Cursor is the one harness where "non-thinking" is selected by picking a different model ID outright, not a flag, which is why effort rides in the id here.

### Gemini (as consumed via each harness)

| Band               | Model                                                                  | Effort |
| ------------------ | ---------------------------------------------------------------------- | ------ |
| cheap              | `gemini-3.6-flash`                                                     | high   |
| standard / max     | `gemini-3.1-pro` (Cursor) / `gemini-3.1-pro-preview` (Vertex, Copilot) | high   |
| counter (on `max`) | none — Google-only catalog                                             | —      |

Gemini is the one harness with no gate adapter: `invoke_agent` takes no model argument, and the `BeforeModel` hook that could rewrite one carries no agent identity. Bands land through `agents.overrides.*.modelConfig.model` in `settings.json`, generated from the registry by `scripts/generate_subagent_models.py`.

Naming is genuinely inconsistent across sources, confirmed live: Cursor's own catalog calls it plain `gemini-3.1-pro` with no `-preview` suffix and no visible effort-tier suffixes; `provider_models` (Vertex) and `copilot_models` both call it `gemini-3.1-pro-preview`. Do not normalize to one spelling — record both.

`gemini-3.6-flash` is a different, newer model than `gemini-3.5-flash` (which is what's actually in `provider_models`/`copilot_models` today). Use `gemini-3.6-flash` where the harness exposes it (confirmed live in Cursor, and defined in the Vertex `provider_models` block); fall back to `gemini-3.5-flash` explicitly where a harness's catalog doesn't yet have 3.6 (e.g. Copilot's snapshot) rather than treating the two as interchangeable.

### Pi

| Band               | Model                                       | Effort |
| ------------------ | ------------------------------------------- | ------ |
| cheap              | `openrouter/openai/gpt-5.2:high`            | high   |
| standard           | `openrouter/openai/gpt-5.2:high`            | high   |
| max                | `openrouter/openai/gpt-5.2:high`            | high   |
| counter (on `max`) | none — one vendor is reachable from Pi here | —      |

All three bands are the same id because Pi reaches models only through OpenRouter, and this repo pins OpenRouter to one model. Refutation therefore runs same-family and the controller reports `families=same (degraded)`.

Pi has no hook system, so profiles are the only lever and the gate never runs there. Pi's default model/thinking is config-file based, but Pi also supports `--model`, `--thinking`, and model strings suffixed with `:<thinking>`. The `,ai` OpenRouter route deliberately rejects those overrides and pins thinking to `high`. `pi-subagents` has per-task/per-agent `model` but no separate per-task `thinking`; use the suffix form (for example, `openrouter/openai/gpt-5.2:high`) for subagent pins. That suffix is a thinking level, not a separate effort dial: `pi-subagents` 0.38.0 parses it with `splitKnownThinkingSuffix` against `THINKING_LEVELS = ["off", "minimal", "low", "medium", "high", "xhigh", "max"]` (`src/shared/model-info.ts`), and `@earendil-works/pi-agent-core` declares the same union in `dist/types.d.ts:254`. So `claude-sonnet-5:high` means thinking high, and `off` is the only non-thinking value — Pi cannot express "high effort, non-thinking" because both live on one dial. The `:<level>` suffix is a thinking dial, not a context tier — Pi exposes no context selector, so every row is short by construction.
...
| ------------------ | ----------------------------------------------------- | -------------------------- | -------------------------- |
| cheap              | `@smol`                                               | `cursor/composer-2.5:high` | `cursor/composer-2.5:high` |
| standard           | `@task`                                               | `cursor/gpt-5.2-high:high` | `cursor/gpt-5.2-high:high` |
| max                | `@default`                                            | `cursor/gpt-5.2-high:high` | `cursor/gpt-5.2-high:high` |
| counter (on `max`) | none — `@advisor` prices to the same id as `@default` | —                          | —                          |

Verified on 17.2.4: a profile carrying `model: "@smol"` runs on `modelRoles.smol`, and an unknown token fails loudly with `Error: No model selected.` rather than falling back. Provider and model are separated by `/`, never `:` — `cursor:` parses as a bogus provider. Like Pi, OMP's `:<level>` suffix is a single thinking dial the runtime maps straight onto `reasoning`, so "high effort, non-thinking" is not expressible here. Both profiles pin their primary roles to Cursor `gpt-5.2-high`, use `composer-2.5` for `smol`, and use Cursor `claude-sonnet-5-high` for `vision`.

`modelRoles.advisor` resolves to the same `cursor/gpt-5.2-high:high` as `default` on both profiles, so there is no second family to refute with: `max` carries no counter, both review-lane roles name `@default`, and refutation is `families=same (degraded)`. An invariant asserts OMP carries a counter only while `advisor` differs from `default`, so restoring cross-family refutation means repricing the role first, not adding the band back.

## Native subagent takeover risk

Every harness that can spawn subagents has its own **native** default model for that path — separate from anything this repo's registry declares — and an unpinned harness silently falls back to whatever that native default is. This is the risk this taxonomy exists to close, not just document. `agent_bindings` therefore lists built-in names (Copilot's `explore`, Codex's `worker`, Gemini's `generalist`, Cursor's `generalPurpose`) next to the repo-authored profiles, and the gate covers the call sites that no profile can reach.

| Harness         | Takeover risk                                                                                                                                                                                                         | Status                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| --------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Cursor**      | Omitted/default Cursor subagents are unsafe: local CLI shows `auto`, and user-verified expenditure data shows Cursor-served defaults can resolve to `composer-2.5-fast`                                               | **Mitigated by the gate**, which is the only mechanism available: `cursor-agent` does not read `~/.cursor/agents`, and same-name profiles cannot shadow native enum agents in a workspace either. The `preToolUse` hook rewrites `Task.model` for every bound `subagent_type`, within the Task tool's own model whitelist. Cursor `worker`/cloud `requested_models` omission remains an open risk unless launched with explicit models. |
...
| **Claude Code** | `Task` tool w/ `subagent_type`, embedded builtins (`Explore`, `Plan`, `general-purpose`, `claude-code-guide`, `claude`), plus a separate `claude agents`/background-agent surface with its own `--model` default flag | **Mitigated for the strict OpenRouter route**: repo-owned same-name profiles shadow high-risk builtins, and `,claude-openrouter` pins `CLAUDE_CODE_SUBAGENT_MODEL`, all family defaults, and root invocation to GPT-5.2 at `high`.                                                                                                                                                                                                      |
| **Gemini**      | `.gemini/agents/*.md` + `@name` forced invocation                                                                                                                                                                     | **Mitigated by configuration only** — profiles carry registry-rendered `model` frontmatter and built-ins are pinned through generated `agents.overrides`. There is no gate here: `invoke_agent` accepts no model argument.                                                                                                                                                                                                              |
| **Pi**          | `~/.pi/agent/agents/*.md`; subagent runner accepts model strings and encodes thinking as a suffix                                                                                                                     | **Mitigated for repo-owned profiles** — Pi profiles render from `model_bands`/`agent_review_models`; lanes and verifier both use `openrouter/openai/gpt-5.2:high`. Pi has no hook, so future profiles must either pin `model` or document why they intentionally use `defaultProvider`/`defaultModel`.                                                                                                                                  |
