...
| Review / Adversarial verification / Findings audit / Post-act verification | `claude-opus-5`   | high   | off      | long    |
...
| Review / Adversarial verification / Findings audit / Post-act verification | `claude-opus-5` (high, non-thinking) **or** `gpt-5.5` (medium) | —                   |

Copilot's live 1.0.75 catalog confirms `gpt-5.3-codex` and `claude-opus-5`, so Copilot uses them directly for gruntwork and review respectively. Sonnet 5 remains available for routine editing, while orchestration/design use `gpt-5.5` at medium effort. Model IDs stay catalog-native: Copilot uses dotted 4.x IDs such as `claude-opus-4.8`, but the Opus 5 ID is `claude-opus-5`.
...
Cursor's live catalog confirms `gpt-5.3-codex`, so Cursor gruntwork uses it directly. `glm-5.2-high`/`glm-5.2-max` are still real model IDs, but they are no longer the default gruntwork pick. `claude-opus-5-high` (plain) and `claude-opus-5-thinking-high` are distinct, real model IDs — Cursor is the one harness where "non-thinking" is selected by picking a different model ID outright, not a flag.
...
| Review / Adversarial verification / Findings audit / Post-act verification | `openrouter/anthropic/claude-opus-5:high` (non-thinking) **or** `openrouter/openai/gpt-5.5:medium` | —                   |
