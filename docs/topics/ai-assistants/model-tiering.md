...
DeepSeek max carries every Pi primary band with FP8-or-higher quantization and throughput-only provider sorting. Kimi remains selectable only on Fireworks, Together, and BaseTen with dynamic throughput sorting, fallbacks disabled, and a $16/M completion cap; the counter stays `gpt-5.6-terra:max`, so lanes vs counter remain cross-family.
