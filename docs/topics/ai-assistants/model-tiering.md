...
**Two standing policies keep the option space small.** Every category runs short context unless the harness publishes no short variant of the wanted model, and `lookup` is exact retrieval only rather than semantic discovery. Copilot uses `gpt-5.5`/xhigh for every category except mechanical (`claude-sonnet-4.6`/high) and refute (`claude-fable-5.1`/high). Cursor mirrors that shape only where its Task enum can spawn the id: primary categories use `gpt-5.6-sol-high`, the cheap lanes (mechanical, memory) use the `auto` router selector, and refute uses `claude-fable-5-1-thinking-high`. Antigravity uses `gemini-3.1-pro-preview` long-context categories except mechanical, which stays on `gemini-3.8-flash`; Claude Code and Antigravity keep single-vendor picks, and native Codex deliberately uses `gpt-5.4`/high for mechanical while every other category uses `gpt-5.5`/xhigh.
...
| --------------------------------- | ------------------- | ------ | -------- | ------- | --------------- |
| `lookup`                          | `claude-fable-5-1`  | low    | off      | short   | —               |
| `mechanical`                      | `claude-sonnet-4-6` | high   | off      | short   | —               |
| `memory`                          | `claude-sonnet-4-6` | low    | off      | short   | —               |
| `research`, `implement`, `review` | `claude-fable-5-1`  | medium | off      | short   | —               |
| `orchestrate`                     | `claude-fable-5-1`  | xhigh  | off      | short   | —               |
| `refute`                          | `claude-fable-5-1`  | medium | off      | short   | degraded        |

Claude Code cannot take a cross-vendor counter: it accepts only Claude-family selectors, and an unknown id is not remapped — it reaches the API and returns `API model not found`. `refute` therefore resolves to Fable 5.1 and reports `degraded`. Fable carries lookup and conclusion-forming categories (Fable 5 user call 2026-08-05, following the Cursor GPT-5.6 exit; bumped to 5.1 on 2026-09-01); mechanical work uses Claude Sonnet 4.6 at high effort. The Claude Code selector is `claude-sonnet-4-6`, while Copilot uses dotted `claude-sonnet-4.6`. The 2026-08-03 opus-ban exemption for this harness is superseded — Opus is no longer an active pick here. `claude-fable-5` is verified inside the installed bundle (2.1.222, still present in 2.1.241) and `fable` is a first-class Agent-tool alias there; `claude-fable-5-1` follows Claude Code's hyphenated point-version convention but is absent from the 2.1.241 bundle and unverified live, because the API-key route short-circuits on billing before model validation. `alwaysThinkingEnabled: false` keeps the pick non-thinking.
...
Copilot's captured catalog includes `gpt-5.5`, `claude-sonnet-4.6`, and `claude-fable-5`; `claude-fable-5.1` is listed ahead of org enablement (requested 2026-09-01; a `copilot -p --model claude-fable-5.1` probe on CLI 1.0.82 that day still returned "not available"). Copilot CLI 1.0.80 accepts `xhigh` as a reasoning effort. User call 2026-08-24: Copilot defaults categories to `gpt-5.5`/xhigh, with `mechanical` on `claude-sonnet-4.6`/high and `refute` on `claude-fable-5.1`/high. Effort is not an agent _frontmatter_ field, but `~/.copilot/settings.json` carries `subagents.agents.<name>.effortLevel`, so per-lane effort is pinnable there. That file is source JSON the merge script reads and the artifact ledger records as a `json-declared` baseline path, so it cannot be a template; `scripts/generate_subagent_models.py` writes it from the category registry instead and an invariant fails when the two drift. Model IDs stay catalog-native: Copilot uses dotted 4.x IDs such as `claude-opus-4.8`, but the Opus 5 ID is `claude-opus-5`.
...
| ---------------------------------------------------------- | -------------------------------- | ------ | ------- | --------------- |
| `lookup`, `research`, `implement`, `orchestrate`, `review` | `gpt-5.6-sol-high`               | high   | long    | —               |
| `mechanical`                                               | `auto`                           | —      | short   | —               |
| `memory`                                                   | `auto`                           | —      | short   | —               |
...
| ------------------------------------- | ---------- | ----------------------------------------- | ----------------------------------------- |
| `lookup`                              | `@smol`    | `cursor/default` (Auto router)            | `cursor/default` (Auto router)            |
| `memory` (direct id, not a role)      | —          | `openrouter/google/gemini-3.7-flash:high` | `openrouter/google/gemini-3.7-flash:high` |
| `mechanical`, `research`, `implement` | `@task`    | `cursor/gpt-5.5:xhigh`                    | `openai-codex/gpt-5.5:xhigh`              |
| `orchestrate`                         | `@plan`    | `cursor/gpt-5.5:xhigh`                    | `openai-codex/gpt-5.5:xhigh`              |
| `review`, `refute`                    | `@advisor` | `cursor/claude-opus-5-high:high`          | `openai-codex/gpt-5.5:xhigh`              |

Verified on 17.2.4: a profile carrying `model: "@smol"` runs on `modelRoles.smol`, and an unknown token fails loudly with `Error: No model selected.` rather than falling back. Provider and model are separated by `/`, never `:` — `cursor:` parses as a bogus provider. Like Pi, OMP's `:<level>` suffix is a single thinking dial the runtime maps straight onto `reasoning`, so "high effort, non-thinking" is not expressible here. User call 2026-08-30: the work profile rides the Cursor backend — default/vision/slow/plan/task on `cursor/gpt-5.5:xhigh` (unlike `category_models.cursor`, which stays on `gpt-5.6-sol-high` only because the Cursor Task enum lacks gpt-5.5), `smol` on `cursor/default` (the discovered cursor catalog's "Auto" router id; reasoning off, so no `:level` suffix), and `advisor` on `cursor/claude-opus-5-high:high`; all three routes probed live on omp/18.0.10 the same day. The personal profile rides the Codex backend (same user call): default/vision/slow/plan/task and `advisor` on `openai-codex/gpt-5.5:xhigh` (the openai-codex catalog is OpenAI-only, matching the codex harness rows), and `smol` on `cursor/default`. The `openai-codex/gpt-5.5:xhigh` route was probed live on the work box; the personal machine's own codex/cursor auth is not locally verifiable from there.

`memory` bypasses the role table on purpose: DeepSeek V4 Flash (then `modelRoles.smol`) failed the live scribe probes (stored a known duplicate on Pi; hung as OMP scribe, killed at 9 min, 2026-08-28), while Gemini 3.7 Flash returned the correct `duplicate of <id>` on the same fixture (2026-08-29). Pinning the direct id keeps `modelRoles.smol` — and the `lookup` lane riding it — unchanged. Gemini 3.7 Flash tops out at `high` (no `xhigh`). The `:high` suffix in the row is load-bearing: [`agent-model.partial`](../../../home/.chezmoitemplates/agent-model.partial) renders only the model string into the agent frontmatter (the registry `effort` field is never rendered for OMP), and OMP's spawn precedence honors an explicit `:level` suffix over its defaults (`task/executor.ts`: effort > `:level` suffix > agent-definition default > pattern-derived).

`modelRoles.advisor` is enabled for primary turns and spawned agents. Review and refute both resolve `@advisor` (Opus 5 High on work, GPT-5.5 on personal — the same shared role either way), and `verifier_status: reduced_independence` makes that reporting explicit.
