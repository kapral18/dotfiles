...
| Session hooks      | `ai-kb-recall.ts` invokes the shared session-context hook, stages depth-aware per-turn recall candidates for the smol judge (pointer injection only) plus the correction directive, and forwards tool results to the shared worklog. |
