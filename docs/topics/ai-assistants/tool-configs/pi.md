...
| Secrets            | `GEMINI_API_KEY`, `OPENROUTER_API_KEY` and `NVIDIA_API_KEY` (pass entry `nvidia/nim/api/token`, for the built-in `nvidia` NIM provider) are picked up from environment variables exported via `pass` in `config.fish.tmpl`. `ANTHROPIC_API_KEY` and `OPENAI_API_KEY` are not exported; those are subscription logins, and any tool needing a raw key reads `pass` itself.                                                                                                                                                                                       |
...
`80000` is sized for the 262144-token local Nemotron window, ~30% recent-verbatim. It also remains below the work-profile Qwen3.6 131072-token context and the configured OpenRouter default's context limit.
...
For GPT models without tier metadata, short uses a 272,000-token working budget, not a guarantee about their pricing.
...
Grok 4.5/4.6 are eligible (500k native, 200k tier). Claude Fable is not.
...
Native sources (installed Pi 0.87.1 / pi-subagents 0.70.1, under the pnpm global store): the adapter loads the single entry the package declares in `package.json` `pi.extensions` (compiled `./index.js` since 0.70.0) and refuses to register anything when that entry is missing or ambiguous; the factory projection point is `pi-subagents/src/extension/index.js` (the `subagent` tool registration, with parameters from `createSubagentParamsSchema` in `src/extension/schemas.js`); package filtering is `pi-coding-agent/dist/core/package-manager.js` (`collectPackageResources`/`applyPackageFilter`); the validator-before-hook order is `pi-agent-core/dist/agent-loop.js` (`prepareToolCall` runs pi-ai `validateToolArguments` from `pi-ai/dist/utils/validation.js` before `beforeToolCall`, and the failure text is `Validation failed for tool "subagent"`); single-owner registration rests on `pi-coding-agent/dist/core/extensions/runner.js` (`getAllRegisteredTools`, first registration per name wins) plus the filter above.
