...
| [Other harnesses](other-harnesses.md)               | Codex, OpenCode, Copilot, tuicr, lgtm, secrets                                       |
...
| --------------------- | --------------------------------------------------------------------------------------------- | ----------------------------------------------------- | ------------------------------------------- | ------------- |
| `codex`               | OpenAI's native CLI runs coding sessions without the managed local-model catalog injection.   | `gpt-6-astra`                                         | high                                        | short         |
| `,codex`              | The managed wrapper injects metadata for selected local models before launching native Codex. | `gpt-6-astra`                                         | high                                        | short         |
| `,ai codex`           | The unified launcher selects `,codex` while preserving its default model and effort.          | `gpt-6-astra`                                         | high                                        | short         |
| `,codex-copilot`      | This wrapper runs Codex through the GitHub Copilot subscription adapter.                      | `claude-fable-5.1`                                    | high                                        | long          |
...
| `copilot`             | GitHub's native CLI runs coding sessions without the managed resume preprocessing.            | `claude-fable-5.1`                                    | high                                        | long          |
| `,copilot`            | The managed wrapper preselects bare resumes and disables Anthropic thinking by default.       | `claude-fable-5.1`                                    | high; Anthropic thinking disabled           | long          |
| `,ai copilot`         | The unified launcher selects `,copilot`, retaining its resume handling and thinking policy.   | `claude-fable-5.1`                                    | high; Anthropic thinking disabled           | long          |
| `,copilot-codex`      | This wrapper runs Copilot through the Codex subscription adapter.                             | `gpt-6-astra`                                         | inherits Copilot high                       | Codex-derived |
| `,copilot-openrouter` | This wrapper runs Copilot through OpenRouter's Messages endpoint.                             | `z-ai/glm-5.3-flash`                                  | high                                        | long          |
| `claude`              | Anthropic's native terminal coding agent reads, edits, and runs project code.                 | `claude-fable-5-1[1m]`                                | high; thinking off                          | long          |
| `,ai claude`          | The unified launcher selects native Claude Code without replacing its model or effort.        | `claude-fable-5-1[1m]`                                | high; thinking off                          | long          |
| `,claude-codex`       | This wrapper runs Claude Code through the Codex subscription adapter.                         | `gpt-6-astra`                                         | inherits Claude high; frontend thinking off | Codex-derived |
| `,claude-copilot`     | This wrapper runs Claude Code through the GitHub Copilot subscription adapter.                | `claude-fable-5.1`                                    | inherits Claude high; frontend thinking off | long          |
...
| `,cursor-codex`       | This wrapper runs Cursor's local-agent frontend through the Codex subscription adapter.       | `gpt-6-astra`                                         | frontend-controlled; unpinned               | Codex-derived |
| `,cursor-copilot`     | This wrapper runs Cursor's local-agent frontend through the Copilot subscription adapter.     | `claude-fable-5.1`                                    | high                                        | long          |
...
| `pi`                  | Pi provides a terminal coding agent with the shared extension setup.                          | `github-copilot/claude-fable-5.1`                     | high                                        | long          |
| `,ai pi`              | The unified launcher starts Pi while inheriting its native configured defaults.               | `github-copilot/claude-fable-5.1`                     | high                                        | long          |
...
- Native-command rows refer to the executables: interactive [Fish functions](../../../../home/dot_config/fish/readonly_config.fish.tmpl) forward bare `codex`, `copilot`, and `cursor-agent` to their comma wrappers; `command codex`, `command copilot`, and `command cursor-agent` bypass those functions.
- Equal model defaults do not make launchers equivalent: [`,codex`](../../../../home/exact_lib/exact_,codex/main.py) adds local-model metadata, [`,copilot`](../../../../home/exact_lib/exact_,copilot/main.py) preprocesses bare resumes and defaults `COPILOT_DISABLE_ANTHROPIC_THINKING=1`, and [`,cursor`](../../../../home/exact_bin/executable_,cursor) performs prelaunch MCP token checks.
- The `,ai` routes for Cursor, Claude, Codex, OpenCode, and Copilot inherit their selected launcher's defaults; the default `balanced` depth does not force medium effort.
- Native Claude selects the explicit long-context `claude-fable-5-1[1m]` model at high effort with thinking off. Local llama.cpp settings also pin their selected model IDs to `high`, while retaining their local context windows.
...
- The four `*-openrouter` wrappers append `@preset/effort-high` to their default model IDs and default to `long`; explicit `--context short` retains the pricing/context-limited route.
- Copilot-backed wrappers default to Fable 5.1's `long_context` tier; Codex and Cursor apply their default high effort only when the live catalog supports it, while `,claude-copilot` leaves effort unset so Claude inherits its native high setting.
...
The [launcher scripts](../../../../home/exact_bin/), [Codex subscription adapter](../../../../home/exact_lib/exact_,codex-adapter/main.py), [Copilot subscription adapter](../../../../home/exact_lib/exact_,copilot-adapter/main.py), [unified launcher](../../../../home/exact_lib/exact_,ai/main.py), and [one-shot launcher](../../../../home/exact_lib/exact_,q/main.py) own wrapper selections.
Native selections come from [Codex](../../../../home/dot_codex/private_config.work.toml), [Copilot](../../../../home/private_dot_copilot/settings.json), [Claude](../../../../home/dot_claude/settings.work.json), [OpenCode](../../../../home/dot_config/opencode/readonly_opencode.work.jsonc), [Pi](../../../../home/dot_pi/agent/readonly_settings.work.json), and [OMP](../../../../home/dot_omp/private_agent/readonly_config.yml.tmpl) config, plus the live overrides noted above.
