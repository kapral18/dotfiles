...
Both interactive profiles default to `gpt-6-astra` with `model_reasoning_effort = "high"`, generated from `session_models.codex` by `scripts/generate_session_models.py`. Native `default` binds to the `research` category. Research, review, and refute use `gpt-6-astra`/high; `k-agent-smol` and `k-agent-mechanical` use `gpt-5.6-luna`/high, while implement workers use `gpt-5.6-terra`/high. Every profile pins `service_tier = "default"`.
...
`,copilot-openrouter` uses the cache-capable `COPILOT_PROVIDER_TYPE=anthropic` client against `https://openrouter.ai/api` with no `/v1` suffix. The display model is the selected OpenRouter id, while `COPILOT_PROVIDER_WIRE_MODEL` carries the root `<model>@preset/effort-<level>` id and delegated lanes are rewritten only when their Pi category row names an OpenRouter model. `COPILOT_PROVIDER_MAX_PROMPT_TOKENS` and `COPILOT_PROVIDER_MAX_OUTPUT_TOKENS` use separate resolved limits. Explicit OpenRouter budget overrides must fit the selected tier and provider capacity.
...
| `,copilot-codex`                                                                    | Root tool interoperability observed; exact child routing remains unverified; configured admission denial observed with the SDK extension loaded | Root adapter use does not prove child caching or a governed leaf lifecycle                                           |
...
| `,cursor-copilot`                                                                   | Authenticated native loopback exchange observed; local Task bypasses hooks; live backend and exact routing not certified                        | Configured denial is not enforced by local Task; no child caching claim                                              |
...
A scripted local-provider check with Copilot CLI `1.0.83` preserved an exact child selector on its Anthropic BYOK path, but the child still exposed `task`. This is not proof of the Responses subscription path or native no-nesting. A separate native offline differential showed why an earlier Responses-path observation could not settle selector behavior: without `enabledFeatureFlags.EXTENSIONS`, Copilot dispatched a `claude-opus-5` task without calling the gate; with only that flag enabled, the original managed SDK extension called the deployed band gate and the configured subscription policy denied before any child request. Exact role/model/effort routing on `,copilot-codex` therefore remains unverified rather than proven absent.
The adapter had also dropped every tool call on that path, because it relayed a terminal `response.completed` whose `output` array was empty; it now restores the streamed items, so root `bash` and `task` surfaces run. Root tool interoperability does not certify successful governed delegation or a complete leaf lifecycle.
Isolated-home Cursor `2026.09.08-6caf4ff` checks stopped at authentication before any loopback request. Reusing the existing native Cursor login, scripted-provider checks under both subscription launch environments executed a child and returned its result without invoking the deployed user gate or an instrumented workspace gate. The installed local-provider function `nhe()` sets `enableExecuteHookExec:false`; the Task hook path therefore returns before calling the executor. This is a native admission bypass, not evidence of exact child routing or live Copilot backend acceptance. Codex `0.154.0` natively supports reopening closed agents, so removing child collaboration tools does not seal terminal results against root revival. See the [route capability matrix](../subagents.md#coverage-inventory) and do not assign unattended child work to an uncertified route.
...
Without `--context`, all three wrappers use Copilot's `long_context` tier. Explicit `--context default` selects the model's default tier instead. When the selected model advertises `long_context`, that tier caps the prompt allowance at the model's input capability before adding its separate output allowance to the context metadata. Models without that tier fail before the child starts. This matches Copilot CLI's [`--context` contract](https://docs.github.com/en/copilot/reference/copilot-cli-reference/cli-command-reference).
...
The typed reconciler recursively preserves live keys absent from the baseline and lets declared values win. The native root is `claude-fable-5.1` at `high` with `contextTier: "long_context"`, generated from `session_models.copilot`. It replaces `subagents.agents` exactly with the declared agent map, which preserves the existing child selections while removing stale agent names and persisted per-agent model/effort/context overrides; unrelated runtime preferences survive. The baseline sets only `enabledFeatureFlags.EXTENSIONS=true` among feature flags; Copilot CLI 1.0.83 returns before SDK extension discovery unless that typed flag resolves true, while unrelated live feature flags remain runtime-owned.
...
The active context, read gate, band gate, and worklog path is the `agent-memory` SDK extension. It registers `onSessionStart`, `onUserPromptSubmitted`, `onPreToolUse`, `onPostToolUse`, and `onPostToolUseFailure`, translates Copilot's camelCase SDK payloads to the shared snake_case script contract, and returns SDK `additionalContext`. Copilot CLI 1.0.83 loads it only when `enabledFeatureFlags.EXTENSIONS` resolves true.
