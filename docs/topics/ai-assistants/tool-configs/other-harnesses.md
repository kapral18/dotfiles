...
Both interactive profiles default to `gpt-6-sol` with `model_reasoning_effort = "high"`, generated from `session_models.codex` by `scripts/generate_session_models.py`. Native `default` binds to the `research` category. Research and review use `gpt-6-sol`/high, and refute uses the same `gpt-6-sol`/high; `k-agent-mechanical` uses `gpt-6-luna`/high and `k-agent-smol` (memory) uses `gpt-6-sol`/high, while implement workers use `gpt-6-sol`/medium. Every profile pins `service_tier = "default"`.
...
Delegated lanes need their own backend schema. The Codex, Cursor, and Claude OpenRouter wrappers export `AGENT_BAND_SCHEMA_HARNESS=pi` and `AGENT_BAND_MODEL_FORMAT=openrouter-preset`, so [the band gate](../model-tiering.md) resolves every bound delegated agent through Pi's OpenRouter category matrix and converts the result to the wrapper's preset wire format. Claude Code applies the same backend schema through alias defaults because its Agent tool accepts family aliases, not arbitrary OpenRouter ids.
...
| `,cursor-codex`                                                                     | Prior root shell smoke; authenticated local Task bypasses configured hooks; exact child routing unverified                              | Configured denial is not enforced by local Task; root smoke does not prove child caching                             |
...
The adapter had also dropped every tool call on the Responses path, because it relayed a terminal `response.completed` whose `output` array was empty; it now restores the streamed items, so root `bash` and `task` surfaces run. Root tool interoperability does not certify successful governed delegation or a complete leaf lifecycle.
Isolated-home Cursor `2026.09.08-6caf4ff` checks stopped at authentication before any loopback request. Reusing the existing native Cursor login, scripted-provider checks under the subscription launch environment executed a child and returned its result without invoking the deployed user gate or an instrumented workspace gate. The installed local-provider function `nhe()` sets `enableExecuteHookExec:false`; the Task hook path therefore returns before calling the executor. This is a native admission bypass, not evidence of exact child routing. Codex `0.154.0` natively supports reopening closed agents, so removing child collaboration tools does not seal terminal results against root revival. See the [route capability matrix](../subagents.md#coverage-inventory) and do not assign unattended child work to an uncertified route.
...
`,claude-codex` and `,cursor-codex` start one authenticated adapter on a random `127.0.0.1` port and stop it with the harness. The child receives only a random per-launch loopback token. The adapter reads the existing Codex ChatGPT OAuth state from `${CODEX_HOME:-~/.codex}/auth.json`; it never copies the upstream access token into the Claude or Cursor environment.
...
| `,cursor-codex` | OpenAI Responses or Chat Completions | accepts Responses directly or translates Chat Completions requests and SSE/JSON responses; the authenticated scripted local frontend selected Responses |
...
`readonly_config.yml.tmpl` is the complete declarative OMP contract. Native `extendedContext: true` gives the root its long context while every role pin remains unchanged. `modelRoles` is one profile-independent block placed before the `isWork` branch (user call 2026-09-07); only the web-search chains still fork per profile. Every built-in role is pinned explicitly so nothing falls through to the harness default or the `@smol` fallback: every role rides the `openai-codex` provider and mirrors `category_models.codex` (user call 2026-09-23): `default` on `openai-codex/gpt-6-sol:high`, `slow` and `plan` on `:max` (T1), and `vision` on `openai-codex/gpt-6-luna:high`; `task` on `openai-codex/gpt-6-sol:medium` (T2 implement — the native `task` agent and every implement worker); `smol` on `openai-codex/gpt-6-luna:high`; `tiny` (titles, memory, auto-thinking classification, unexpected-stop detection) on `openai-codex/gpt-6-luna:medium`; `commit` on `openai-codex/gpt-6-luna:medium`; `advisor` on `openai-codex/gpt-6-sol:high`. `omp config list --json` reports the effective typed settings; inspect all model-role pins together with `omp config get modelRoles`, not with dotted child keys. A second file, `readonly_models.yml` (`~/.omp/agent/models.yml`), keeps the bare OpenRouter ids selectable and declares the preset-slug ids used by shared provider-policy routes: `z-ai/glm-5.3-flash@preset/glm-lanes-high` (FP8-or-higher quantization, 24 t/s floor, no `sort` so default load balancing keeps uptime), `z-ai/glm-5.2@preset/glm-lanes-max` (same FP8-or-higher, 24 t/s floor policy; still selectable), `deepseek/deepseek-v4.1-flash@preset/deepseek-lanes-max` (35 t/s preferred floor under a $1.20/M completion cap, no quantization filter; still selectable), and `moonshotai/kimi-k3@preset/kimi-lanes` (Fireworks/Together/BaseTen-only under the $16/M cap), with `auth: none` so the env `OPENROUTER_API_KEY` still authorizes (see [Model registry & routing](../model-registry.md#openrouter-routing)).

| Setting                                                                                | Work value                                                                                          | Personal value                                             |
| -------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------- | ---------------------------------------------------------- |
| `modelRoles.default`, `vision`, `slow`, `plan` (T1)                                    | `openai-codex/gpt-6-sol` (`:high` default, `:max` slow/plan); vision `openai-codex/gpt-6-luna:high` | same (one profile-independent block)                       |
| `modelRoles.task` (T2 implement: native `task` agent + implement workers)              | `openai-codex/gpt-6-sol:medium`                                                                     | same (one profile-independent block)                       |
...
| `modelRoles.advisor`                                                                   | `openai-codex/gpt-6-sol:high`                                                                       | same                                                       |
