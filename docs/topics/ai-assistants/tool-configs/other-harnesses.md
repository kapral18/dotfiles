...
`,claude-openrouter` points `ANTHROPIC_BASE_URL` at `https://openrouter.ai/api` with no `/v1` suffix, because Claude's Anthropic SDK appends `/v1/messages`. It keeps the root model on the selected OpenRouter wire id, maps Claude aliases onto the Pi backend lanes (`fable`/`opus` → GPT-5.5 xhigh, `haiku` → DeepSeek V4 Flash xhigh, `sonnet` → Sonnet 4.6 xhigh), passes the client-side `--effort high` gate, sets `ANTHROPIC_AUTH_TOKEN`, and clears `ANTHROPIC_API_KEY`.
