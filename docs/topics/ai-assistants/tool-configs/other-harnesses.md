...
`,claude-openrouter` points `ANTHROPIC_BASE_URL` at `https://openrouter.ai/api` with no `/v1` suffix, because Claude's Anthropic SDK appends `/v1/messages`. It pins the root model, all family defaults, and `CLAUDE_CODE_SUBAGENT_MODEL` to `deepseek/deepseek-v4-flash-0731@preset/effort-max`; it also passes the client-side `--effort high` gate, sets `ANTHROPIC_AUTH_TOKEN`, and clears `ANTHROPIC_API_KEY`.
