...
`,claude-openrouter` points `ANTHROPIC_BASE_URL` at `https://openrouter.ai/api` with no `/v1` suffix, because Claude's Anthropic SDK appends `/v1/messages`. It keeps the root model on the selected OpenRouter wire id and fills its four Claude alias slots with fixed backend wire ids along the tier ladder — `fable` → Fable 5.1 high (T1), `opus` → GPT-5.6 SOL high (T2 implement), `sonnet` → GLM 5.3 Flash high (mechanical), `haiku` → Gemini 3.8 Flash low (memory) — then projects every Pi backend pick onto the slot its model family matches (`band_gate.py`'s `_claude_alias_for_backend_model`). Five Pi picks share four slots, so the refute pick (`openai/gpt-5.6-sol` xhigh) lands on `opus`: the alias carries high rather than xhigh, so the band hook denies that launch. Do not report the unavailable pair as an equivalent refutation lane. `CLAUDE_CODE_SUBAGENT_MODEL`, the fallback for spawns the gate leaves unbound, carries that same T2 id. The wrapper passes the selected effort through Claude's client gate when Claude supports that level (`low` through `max`). OpenRouter `none`/`minimal` presets use Claude's `low` client gate while the model preset carries the real effort. For context, it keeps the model id bare and sets `CLAUDE_CODE_AUTO_COMPACT_WINDOW` to the resolved OpenRouter context tier. The wrapper sets `ANTHROPIC_AUTH_TOKEN` and clears `ANTHROPIC_API_KEY`.
...
Native Claude Code Messages traffic and native Codex Responses traffic remain pass-through, with launch-level effort/thinking controls injected where the selected backend supports them. A raw model change does not imply a child role. Only explicit lane tags bypass root controls; Claude aliases carry them. Codex/Cursor subscription frontends reject delegation until native child-tag transport is verified. For a cross-protocol selection, the loopback reuses the repo's tested subscription translation modules: Messages ↔ Responses for Claude/GPT crossings, and either native harness protocol ↔ Chat Completions for models such as Gemini. Tool calls, tool results, streaming terminal events, effort, Claude backend thinking mode, and provider-owned opaque tool context are translated with the conversation. Opaque context remains in bounded process memory for the lifetime of the adapter; the GitHub bearer remains confined to that process.

The Codex adapter also keys the prompt cache the way the Codex CLI does: one `session_id` header and matching `prompt_cache_key` per adapter process. Live-verified 2026-09-06: a repeated 2.8k-token prompt read 0 cached tokens without them and 2,688 of 2,823 with them, so every translated call used to pay full price. No `originator` impersonation is needed or sent.

Both subscription adapters carry provider cache accounting through every usage shape they emit. Chat and Responses upstreams report `prompt_tokens` / `input_tokens` inclusive of cached and cache-written tokens under `prompt_tokens_details` / `input_tokens_details`; the adapters subtract those into the Anthropic split (`input_tokens` fresh, `cache_read_input_tokens`, `cache_creation_input_tokens`) and re-add them for Chat/Responses frontends, so `,ai-usage` and the frontend transcripts see real hit rates instead of zeros. Fields a backend does not report stay absent rather than zero-filled.
...
Claude's auto-compact window is the selected tier's catalogued prompt limit minus `32k` of headroom (floored at `100k`), so `CLAUDE_CODE_AUTO_COMPACT_WINDOW` keeps every request inside the billed tier. Copilot bills the `long_context` rate by prompt size above the `default` tier's prompt limit (`272k` for GPT-6 Astra, `200k` for Claude models), and Claude Code compacts on the previous response's usage, so the headroom absorbs the turn that would otherwise cross the limit. Windows above Claude's default `200k` also add Claude's `[1m]` frontend capability marker, which is what lets Claude honor a custom model window above `200k`; the loopback strips that marker before sending the exact catalog model ID to Copilot. The loopback removes only Copilot-incompatible Anthropic beta values, currently Claude Code's `advisor-tool-2026-03-01`, while preserving the rest of a native Messages request. Codex receives a provider-owned `/v1/models` projection of the same live catalog so every translated model retains its prompt limit, reasoning levels, shell, and freeform patch tool metadata instead of falling back.
