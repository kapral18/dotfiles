...
Codex and OpenCode use profile merging with MCP injection. Amp settings are tracked directly. Normal `codex` sessions go through the `~/bin/codex` shim, which routes through `headroom wrap codex` by default unless `HEADROOM_DISABLE=1` is set or the selected model is a local llama.cpp model.
