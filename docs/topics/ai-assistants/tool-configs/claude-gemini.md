...
Paid Gemini API quota for Antigravity CLI is declared in [`home/dot_gemini/antigravity-cli/readonly_settings.policy.json`](../../../../home/dot_gemini/antigravity-cli/readonly_settings.policy.json) and merged into `~/.gemini/antigravity-cli/settings.json` by `07-merge-antigravity-cli-settings` (declared-over-live, like Copilot settings). Policy owns `modelProvider: "gemini"`, `model: "Gemini 3.1 Pro"` (registry default), and `enableTelemetry: false`, and strips any top-level `gcp` pin. Runtime fields such as `trustedWorkspaces`, `permissions`, and `statusLine` survive. Fish still loads `GEMINI_API_KEY` from `pass google/gemini/api/token`; the key alone does not switch providers.
