...
2. For other-authored or unknown-author PRs, `pr-necessity-auditor` is the first blocking worker. It checks whether the PR is sensible, correctly open, still needed, and worth implementation review before any reviewer fan-out starts. It reconstructs author intent from the PR and references, searches related GitHub/Slack context when available, inspects git history, and looks for overlapping open or recently merged cross-cutting work. Review greenlight is separate from merge readiness: unknown mergeability or failing status checks are surfaced as merge-readiness/status uncertainty, not as "no conflicts" and not necessarily as implementation-review blockers. If it returns blocked, unclear, not needed, superseded, or incorrectly open, the controller stops and surfaces PR-level feedback unless the user explicitly asks to continue anyway.
3. After any required PR necessity greenlight, two read-only reviewer workers run in parallel when the harness supports it. Parallelism is for investigation, not shared-state mutation: lanes may run non-mutating verification at whatever depth is needed, including expensive static analysis, deep search, isolated reproductions, and full suites when outputs/caches are read-only or isolated away from shared repo/runtime state. They do not edit the worktree, seed data, start shared services, run generators/formatters/installers, or write repo-local caches/artifacts. If stronger evidence needs mutation, a shared/exclusive runtime, or another contention-prone resource, the worker returns `verification_needed` and the controller runs it serially after aggregation or during the act phase.
...
4. `live-ui-review` checks applicable UI/runtime candidates with Playwriter against a controller-supplied target packet and returns comparison evidence, optional screenshot handoff, `Not applicable`, or a target/branch/runtime/data blocker. `Not applicable` can be per-target for PR-introduced surfaces; missing data is setup work. Generic runtime contracts own the phase shape and safety boundary; domain overlays own concrete targets and data setup. For `elastic/kibana`, `elastic-domain` supplies the Kibana main/feature targets, mapped Elasticsearch endpoints, Dev Tools Console fallback, and the rule that runtime config/startup/restart prerequisites block with user instructions instead of mocks. Focused screenshots go under `/tmp` only when they materially clarify a finding, and the controller reports their paths separately for manual attachment.
...
The controller does not load or run the full review methodology before fan-out. Worker profiles are intentionally read-only, concurrency-safe, and recursion-safe; they load the review skill for methodology in isolated contexts and return candidate findings plus any `verification_needed` they could not safely run. The controller only routes, runs the PR necessity gate, fans out after greenlight, runs live UI, audits findings inline or by delegation, aggregates, filters, reconciles pending-review context for PR modes, and acts after the normal gates.
...
Live UI keeps the old Kibana target packet as the default when no explicit user-provided or repo-documented local/dev target packet exists. The default packet comes from `elastic-domain/references/kibana-live-ui.md`. Non-default targets must be explicit; if neither the default nor an explicit packet can be loaded, the flow blocks instead of inventing targets.
...
Pending-review awareness is part of that PR-mode gate, not a posting afterthought. Before a review draft or pending-review payload is finalized, the agent lists reviews for the PR, selects `state == PENDING` reviews authored by the current authenticated login, reads their draft comments, and compares them with any already-submitted review comments/replies by the same account. The output includes `Pending review reconciliation:` so a later posting session can safely reuse, merge, replace, or drop previous-session feedback instead of creating a second competing review.
...
- **Elastic/Kibana readiness audit CLI**: [`,kbn-pr-audit`](../../../home/exact_bin/executable_,kbn-pr-audit) is a read-only Elastic/Kibana tool-pack check (see [Custom commands](../workflow/custom-commands.md)) that surfaces PR-body/label/thread drift before a reply/resolve/push cycle; it never mutates GitHub.
...
- [Ralph orchestrator](ralph.md) — reviewer/re-reviewer roles can invoke this skill through domain review policies
