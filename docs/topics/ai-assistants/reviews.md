...
4. `live-ui-review` first decides applicability from the changed paths and candidate findings. When UI/runtime behavior is in scope, it checks the configured targets with Playwriter and returns comparison evidence or a target/branch/data blocker. Empty target data is setup work, not non-applicability: the worker must inspect PR/issue media, tests, fixtures, and mocks, try browser/route mocks or existing seeded data, and create minimal isolated local/dev runtime data when that is needed and the harness permits safe local/dev setup. It cleans up seeded data when safe and reports exact leftovers or remaining setup blockers.
