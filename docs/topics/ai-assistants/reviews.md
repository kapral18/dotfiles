...
4. `live-ui-review` checks applicable UI/runtime candidates with Playwriter and returns comparison evidence, optional screenshot handoff, `Not applicable`, or a target/branch/runtime/data blocker. `Not applicable` can be per-target for PR-introduced surfaces; missing data is setup work. Setup order is existing data, real local Kibana/Elasticsearch data, Kibana Dev Tools Console, then route/network mocks as lower-fidelity last resort. Runtime config/startup/restart prerequisites block with user instructions instead of mocks. Focused screenshots go under `/tmp` only when they materially clarify a finding, and the controller reports their paths separately for manual attachment.
...
Its configured targets are `http://kibana-main.local:5602` for base and `http://kibana-feat.local:5601` for PR/head, backed by local Elasticsearch endpoints `http://localhost:9201` and `http://localhost:9200` respectively. It uses Playwriter target checks only when the applicability gate says UI/runtime behavior is in scope.
