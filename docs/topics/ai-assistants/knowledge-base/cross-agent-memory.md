...
`,ai-kb` is the only durable store. Native harness auto-memory stays off so a second, harness-local store never forms: Claude Code `autoMemoryEnabled: false`, Codex `memories = false`, OMP `memory.backend: off`.
