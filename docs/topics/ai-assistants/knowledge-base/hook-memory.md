...
| [`perturn_recall.py`](../../../../home/exact_dot_agents/exact_hooks/)                             | Per-prompt hook: AI-KB recall on harnesses that support context injection per turn  |
...
- `,agent-memory note <kind> "<text>" [--ref <anchor>]` — record a structured insight into the bound topic's worklog through the crash-safe queue. Kinds are the `,ai-kb` capsule kinds (minus `doc`) plus task-scoped `question`; non-question notes surface later as `,ai-kb harvest` candidates with their kind kept verbatim.
...
| ----------- | ---------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------- |
| Cursor      | [`hooks.json`](../../../../home/dot_cursor/hooks.json) → `~/.cursor/hooks.json`          | `sessionStart` → `session_context.py`                                                                                                       | — (see note)                                                        | `afterShellExecution`, `postToolUse`, `postToolUseFailure`, `afterFileEdit` → `worklog_dispatcher.sh`                   |
...
- **Cursor** has no per-prompt context-injection channel: `beforeSubmitPrompt` output is allow/block only (read as `{continue, user_message}`; verified in the installed 2026.07.09 `cursor-agent` bundle). `session_context.py` therefore appends a `### Recall Notice` telling the model to run its own `,ai-kb search` mid-task. The notice keys off the warm-up signal: an adapter that neither sets `AI_EMBED_WARM=1` nor sends `warm_embedder: true` is treated as having no per-turn recall.
- **Codex executes hook commands without a shell**: a literal `$HOME/...` never expands and the hook fails with `hook: <name> Failed`. The adapter is a chezmoi template that renders absolute paths; env for the warm-up is set through an explicit `/bin/sh -c` wrapper. Codex also strictly validates hook results and rejects unknown top-level keys, so the wrapper sets `AGENT_HOOK_OUTPUT=hook_specific`, which makes the shared `emit()` keep only the `hookSpecificOutput` channel (the top-level `additional_context` key exists for Cursor, which reads nothing else). Payload shapes are Claude-compatible (verified against codex 0.144.4): `SessionStart` and `UserPromptSubmit` carry `session_id`, `cwd`, and `prompt`, and `hookSpecificOutput.additionalContext` from `UserPromptSubmit` lands in the transcript as a developer message. `codex exec` dispatches `SessionStart`, `UserPromptSubmit`, and `PostToolUse` like interactive sessions.
