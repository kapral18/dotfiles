...
| Verify a change actually works end-to-end         | `verify`-style live drive; UI via `k-playwriter`, or `k-ui-capture` for screenshot proof of an intended visual | "verify this works" / "screenshot the UI for the PR"                       | rows in [skills](skills/external-tools-and-media.md)                                   |
