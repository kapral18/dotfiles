...
The three questioned subscription routes have separate root, routing, admission, and lifecycle evidence:

| Route             | Root interoperability                                                                    | Exact child role/model/effort routing                                            | Governed admission                                                                                                      | Full leaf lifecycle                                       |
| ----------------- | ---------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------- |
| `,copilot-codex`  | Root requests and tool use observed through the adapter                                  | Unverified; the earlier run without SDK extensions was not a valid selector test | Configured policy denies subscription child delegation; denial was observed only after the managed SDK extension loaded | Unverified; no successful governed child request was made |
...
| `,cursor-copilot` | Authenticated native loopback exchange observed; live Copilot backend not certified here | Unverified; a root-model child ran without admission                             | Bypass observed: the same local Task implementation disables its hooks                                                  | Unsupported for governed delegation                       |

For Copilot CLI 1.0.83, `enabledFeatureFlags.EXTENSIONS=true` is a prerequisite for loading the managed SDK extension. In the native offline differential, omitting it let a task request use `claude-opus-5` without a gate call; enabling only that targeted flag made the original managed extension invoke the deployed band gate, which denied the configured subscription route before any child request. This proves effective denial when the extension loads, not selector impossibility, successful governed delegation, or complete leaf lifecycle behavior. The adapter's restored streamed output keeps root tool interoperability separate from those child claims.

Cursor `2026.09.08-6caf4ff` has a separate native limitation. Its local-provider `nhe()` configuration explicitly sets `enableExecuteHookExec:false`; the Task hook path returns without calling the executor. Authenticated scripted-provider probes under both subscription launch environments returned child results without invoking either the deployed user band hook or an instrumented workspace copy. This establishes an admission bypass, not selector rejection. No vendor files were patched, and successful ungoverned child results do not establish role/effort routing.
...
- Pi profiles set `defaultContext: fresh`, `inheritProjectContext: false`, `inheritGlobalContext: false`, `inheritSkills: false`, and child depth zero. This removes ambient instructions/catalog, not explicitly named role skills: pi-subagents 0.66.0 loads those separately in `runs/foreground/execution.ts`. The root supplies applicable project and safety constraints in the packet. Dispatch requires `agentScope:"user"` so project profiles cannot override managed models, tools, or inheritance. The runtime guard requires `acceptance:false` and rejects context/model/per-call skill overrides, composite workflows, host gates, scheduling and resume/steer recovery. Dispatch one ready packet per call; independent root packets may explicitly set `async:true`. The root system prompt states this explicitly and marks the `pi-subagents` tool description's "exactly one top-level subagent workflow call" guidance as superseded, so the package text and the guard no longer conflict at dispatch time. Defaults are fresh and foreground. Native child identity suppresses root memory/reinforcement injection and blocks the subagent tool.
