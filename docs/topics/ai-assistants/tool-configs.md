...
- **Find/judge** is parallel and multi-model after any required greenlight: read-only reviewer workers inspect the diff/PR from files, each on a distinct angle. The fan-out is the place where a second model earns its keep (adversarial disagreement).
- **Findings audit** is proportional: the controller audits trivial zero/one-finding sets inline, and delegates to `findings-auditor` for non-trivial findings, disagreements, blockers, surviving PR-necessity concerns, named fix diffs, or overengineering risk.
