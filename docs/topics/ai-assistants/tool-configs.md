...
| `live-ui-review`                     | `review`               | Verification-only live UI verifier for UI/runtime-relevant candidate findings      |
...
- Workers and auditors are strictly investigation-only for repo, git, GitHub, and publishing surfaces: they report evidence for the parent controller (`/agent-review`, Pi `review-controller`, or Claude main session) to judge rather than editing files, posting, resolving, or deciding themselves. Reviewer lanes are also concurrency-safe: they avoid shared-state mutation, but not deep non-mutating verification, and return `verification_needed` when the parent should run a serial mutation/contention check. Controllers perform the act phase and still honor the Human-Visible Publication Gate (bot threads auto-resolve inside an invoked flow; human threads stop for approval). Before review-posting side effects, controllers also read current-account pending reviews and prior submitted comments/replies, then merge, reuse, or replace them as one coherent payload instead of posting fragmented feedback. `live-ui-review` runs after reviewer fan-out and returns applicability, comparison evidence, or a target/branch/data blocker; missing data is setup work, so the worker tries media/fixture/mocking routes and may seed isolated local/dev runtime data where the harness permits safe local/dev setup before blocking. `findings-auditor` runs after live UI only when proportional.
