...
- **Cursor/Copilot** keep the shared `review` skill as methodology and use the separate `/agent-review` skill as a thin controller. The controller builds a scope packet, fans out to `review-gpt-5-5-extra-high` and `review-opus-4-8-xhigh-non-thinking`, runs `findings-auditor` over the candidate findings, aggregates all three investigation outputs, then judges and acts. The reviewer workers load the full review methodology in their isolated contexts; the controller does not load or duplicate that review pass before fan-out. Cursor pins the lanes at invocation time (`gpt-5.5-extra-high`, `claude-opus-4-8-xhigh`). Copilot uses task subagents with explicit `model` arguments (`gpt-5.5`, `claude-opus-4.8`) because nested probes showed that is the verified child-subagent mechanism.
- **Claude** now has a current-session `agent-review` controller profile. It uses `Task` to call `reviewer` twice with distinct angles and Claude model overrides (`opus`/`sonnet` where available), then calls `findings-auditor`; exact GPT-vs-Opus parity is not available in Claude's verified model override enum.
- **Codex** uses `$CODEX_HOME/agents/*.toml` roles. `review-worker` is launched twice with distinct angles through `spawn_agent`/`wait`, then `findings-auditor`; this preserves isolation and controller-only side effects, but not Opus model diversity.
...
- **Amp** has no custom profile directory in the verified local interface. It bridges by using the generic `Task` subagent tool twice with the shared reviewer contract, then one `Task` for findings audit.
