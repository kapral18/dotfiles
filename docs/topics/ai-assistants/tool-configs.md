...
**Profile defaults:** both work and personal default to provider `openrouter`, model `anthropic/claude-opus-4.8`. The work profile also exposes additional configured models alongside the OpenRouter default. The personal profile additionally exposes `cloudflare-workers-ai` using `CLOUDFLARE_WORKERS_AI_ACCOUNT_ID` and `CLOUDFLARE_WORKERS_AI_API_KEY`, with `@cf/zai-org/glm-5.2` and `@cf/moonshotai/kimi-k2.7-code` as selectable models.
...
The work-profile LiteLLM provider, the personal Cloudflare Workers AI provider, and the local llama.cpp provider for Pi are covered in [Model registry & routing](model-registry.md) and [llama.cpp local inference](llama-cpp.md).
...
**The suite** wraps read-heavy skills (distinct agent names so they never collide with the skill names). `review` is the reference implementation of the phased review pattern; the others are judge-only specializations:

| Agent                                | Wraps skill            | Isolated work                                                                                                                                |
| ------------------------------------ | ---------------------- | -------------------------------------------------------------------------------------------------------------------------------------------- |
| `agent-review`                       | `agent-review`         | `/agent-review` controller: routes, PR-necessity gates, fans out, audits, acts                                                               |
| `review-controller` (Pi)             | `review`               | PR-gates, reviews, audits, then acts (fix tree / draft / drain / verdict)                                                                    |
...
| `findings-auditor`                   | `review`               | Read-only delegated audit for non-trivial findings or a named fix diff                                                                       |
| `live-ui-review`                     | `review`               | Verification-only live UI verifier using a parent-supplied target packet, with default Kibana fallback, including focused screenshot handoff |
...
**Phased review hierarchy** (the topology falls out of the `review` skill's role×mode matrix):

- **PR necessity gate** is first and blocking for other-authored or unknown-author PRs: read the PR, references, history, and overlaps before spending reviewer/UI time. If it does not greenlight the PR as worth implementation review, stop and surface PR-level feedback. Greenlight is distinct from merge readiness: unknown mergeability/status blockers are reported separately and must not be phrased as "no conflicts."
- **Find/judge** is parallel and multi-model after any required greenlight: read-only reviewer workers inspect the diff/PR from files, each on a distinct angle. They may run non-mutating verification at whatever depth is needed, including expensive static analysis, deep search, isolated reproductions, and full suites when outputs/caches are read-only or isolated away from shared repo/runtime state. Any mutation, shared/exclusive runtime, or contention-prone verification is returned as `verification_needed` for the controller to run serially. The fan-out is the place where a second model earns its keep (adversarial disagreement), not a place to race shared-state changes.
- **Findings audit** is proportional: the controller audits trivial zero/one-finding sets inline, and delegates to `findings-auditor` for non-trivial findings, disagreements, material `verification_needed`, blockers, surviving PR-necessity concerns, named fix diffs, or overengineering risk.
- **Act** is serial, side-effectful, and scenario-aware: fix the working tree (own changes / self-review), draft inline comments after current-account pending/submitted review reconciliation (reviewing others), drain threads with the SOP posting gate (PR fix), or emit a verdict (verify-a-fix).
- **Cursor/Copilot** keep the shared `review` skill as methodology and use the separate `/agent-review` skill as a thin controller. The controller builds a scope packet, runs `pr-necessity-auditor` first when required, fans out through configured reviewer lanes only after greenlight, runs conditional live UI verification, audits findings inline or by delegation, aggregates investigation outputs, judges supported findings, reconciles current-account pending/submitted review feedback for PR modes, then acts. The reviewer workers load the full review methodology in their isolated contexts; the controller does not load or duplicate that review pass before fan-out. Cursor pins the default lane in agent frontmatter to `gpt-5.5-extra-high` and the Opus lane to `claude-opus-4-8-xhigh`; generic Subagent/Task calls must pass those same model IDs explicitly. Copilot launches named task agents for its worker profiles; `general-purpose` is only a fallback when named task launch is proven unavailable in the active runtime.
- **Worker selection logging** is required before every delegated phase: exports must show phase, configured profile, task/subagent agent type, model, whether invocation was named or fallback, and any fallback reason.
...
- **Pi** runs the full hierarchy: `review-controller` (default model) detects role+mode, runs the blocking PR necessity audit first for other/unknown PRs, fans out to two `reviewer` tasks with per-task model overrides after greenlight — both via OpenRouter (Pi's `defaultProvider`): `openrouter/anthropic/claude-opus-4.8:off` (no extended thinking) and `openrouter/openai/gpt-5.5:xhigh` (max reasoning budget) — runs live UI when applicable, audits findings inline or by delegation, reconciles via the skill's dedup + truth filter, then performs the act phase itself. The `:<thinking>` model suffix sets reasoning effort; `pi-subagents` does not consume a `thinking` frontmatter field, so effort lives in the model slug, not the agent.
...
- Each agent file's rendered body starts with `prefix.txt`, then explicitly instructs loading the wrapped `~/.agents/skills/<name>/SKILL.md` or a shared runtime contract. Cursor files use the minimal documented `name` + `description` frontmatter and keep model pins in the controller's invocation. Copilot files use `.agent.md` profiles with `target`, `model`, `tools`, and invocation controls; xhigh effort is configured in `settings.json` because `copilot help config` exposes per-subagent `effortLevel`. The Copilot `agent-review` profile is user-invocable; its internal worker/auditor/live profiles are hidden from `/agent` (`user-invocable: false`) but model-invocable (`disable-model-invocation: false`) so the controller can launch named task agents instead of falling back to `general-purpose`. The Cursor/Copilot `agent-review` profiles only load the `/agent-review` skill; worker/auditor/live profiles load `~/.agents/skills/agent-review/references/runtime-contracts.md`, which keeps the per-runtime profiles thin while reviewer workers load the shared `review` methodology in child contexts. Claude files set Claude fields (`model`, `readonly`, `tools`, `skills:` preload) per role; Codex files are TOML role configs with `developer_instructions`; Gemini files are YAML-frontmatter Markdown subagents; Pi files use Pi frontmatter (`inheritSkills`, `systemPromptMode`, comma-separated `tools`, `maxSubagentDepth` on the controller). No `thinking` field — Pi reasoning effort is set per task via the model slug suffix.
- Workers and auditors are strictly investigation-only for repo, git, GitHub, and publishing surfaces: they report evidence for the parent controller (`/agent-review`, Pi `review-controller`, or Claude main session) to judge rather than editing files, posting, resolving, or deciding themselves. Reviewer lanes avoid shared-state mutation, but not deep non-mutating verification, and return `verification_needed` when the parent should run a serial mutation/contention check. Controllers perform the act phase and still honor the Human-Visible Publication Gate. Before review-posting side effects, controllers merge/reuse/drop current-account pending and submitted feedback instead of posting fragmented comments. `live-ui-review` runs after reviewer fan-out and returns applicability, comparison evidence, screenshot handoff, or a target/branch/runtime/data blocker against the selected target packet. Generic runtime contracts own the phase shape; domain overlays own concrete targets and setup ladders. `elastic-domain` supplies the default Kibana/Elasticsearch target packet, Dev Tools Console fallback, lower-fidelity mock rules, and runtime config/startup/restart blockers. For kept UI findings, controllers keep local image paths out of GitHub review bodies and report a separate manual attachment handoff. `findings-auditor` runs after live UI only when proportional.
...
Codex and OpenCode use the profile-merging mechanism above (with MCP injection); Amp settings are tracked directly. The personal OpenCode profile includes a `cloudflare-workers-ai` provider that reads `CLOUDFLARE_WORKERS_AI_ACCOUNT_ID` and `CLOUDFLARE_WORKERS_AI_API_KEY`, so Cloudflare models can be selected as `cloudflare-workers-ai/@cf/zai-org/glm-5.2`, `cloudflare-workers-ai/@cf/moonshotai/kimi-k2.7-code`, or `cloudflare-workers-ai/minimax/m3`. `,opencode-cloudflare` wraps that provider with Fish-completable model choices.

Codex has `,codex-cloudflare`, which targets Cloudflare AI Gateway's OpenAI Responses endpoint. Codex no longer supports Chat Completions wire mode, so this wrapper uses the Gateway OpenAI provider (`CLOUDFLARE_GATEWAY_ID`, default `default`, plus `CLOUDFLARE_API_TOKEN`) rather than Workers AI's `@cf/...` Chat Completions endpoint. Codex and OpenCode also have llama.cpp launcher wrappers (`,codex-llama-cpp`, `,opencode-llama-cpp`) — see [llama.cpp local inference](llama-cpp.md#codex-launcher-codex-llama-cpp). Copilot CLI exposes custom providers only through BYOK environment variables, so `,copilot-cloudflare` and `,copilot-openrouter` set those variables for Cloudflare and OpenRouter's OpenAI-compatible endpoints.

Claude Code is not wired to Cloudflare: its Anthropic-compatible custom base URL path sends `x-api-key` for `ANTHROPIC_API_KEY`, while Cloudflare's `/ai/v1/messages` endpoint requires `Authorization: Bearer ...`; the `ANTHROPIC_AUTH_TOKEN` path did not issue a request in the local probe. A `,claude-cloudflare` wrapper is therefore intentionally not added.
