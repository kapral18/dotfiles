...
For domain-belonging codebases, reviewer and re-reviewer roles can invoke the operator's [`review` skill](reviews.md) directly through a domain review policy. The skill's verification disciplines become the primary instruction; Ralph's existing JSON output contract is preserved as the wire format. Workspaces with no matched domain policy are unchanged.
