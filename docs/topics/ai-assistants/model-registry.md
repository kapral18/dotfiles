...
Upstream provider routing is model-specific (user calls 2026-08-07): every `moonshotai/kimi-k3` route sends `provider: { only: ["fireworks", "together", "baseten"], allow_fallbacks: false, sort: "throughput", max_price: { completion: 16 } }`, while every `deepseek/deepseek-v4-flash-0731` route sends `provider: { sort: "price", quantizations: ["fp8", "fp16", "bf16", "fp32"], preferred_min_throughput: 24 }`; `z-ai/glm-5.2` carries the same price-sorted, FP8-or-higher, 24 t/s floor policy as DeepSeek. The DeepSeek/GLM allowlist excludes INT4/INT8/FP4/FP6 providers; OpenRouter sorts the remaining eligible providers by price (cheapest first) above the 24 t/s throughput floor. See [Provider routing](https://openrouter.ai/docs/guides/routing/provider-selection).

The policies have three carriers. Pi 0.84.0 sends each object through `modelOverrides.compat.openRouterRouting`, which its runtime copies as-is into the request's `provider` field. OMP 17.2.9's typed openrouter transport drops `modelOverrides…compat.extraBody.provider` from the wire (verified 2026-08-08: the merged catalog entry carries no extraBody and a live request with an impossible quantization succeeds), so OMP carries the policy in the model slug through workspace presets like every other provider-field-less harness (the `,claude-openrouter`/`,codex-openrouter`/`,copilot-openrouter`/`,cursor-openrouter` wrappers and OpenCode). The seven `kimi-lanes*` presets preserve their effort-specific `reasoning.effort` values and carry the Kimi object; the six `deepseek-lanes-*` and eight `glm-lanes-*` presets do the same for DeepSeek and GLM-5.2, including the FP8-or-higher quantization allowlist. `,cursor-openrouter` reaches OpenRouter through Cursor's agent-cli-local flavor (`--base-url` OpenAI-compatible), which the regular cursor-agent build rejects; its provider config is baseUrl+apiKey only, so effort rides the preset slug rather than a client dial. Every session runs through the loopback shim, which both strips `strict:true` from `openai/*` tool schemas (cursor-agent-local's reasoning predicate matches those ids and the bundled Shell schema's optional `debounce_ms` fails OpenAI strict validation; see [other harnesses](tool-configs/other-harnesses.md)) and enforces a wire-level model allowlist: the launcher exports the pinned wire id as `CURSOR_AGENT_ALLOWED_MODEL` and the shim returns 403 for any `/chat/completions` whose model differs, so no subagent or profile can route a different model on the session. The wrappers default to `deepseek/deepseek-v4-flash-0731@preset/deepseek-lanes-max` and accept `--model`/`--effort`, composing `<model>@preset/<family>-lanes-<level>` (`kimi-lanes-*`, `deepseek-lanes-*`, `glm-lanes-*`, `terra-lanes-*`; any other model uses `effort-<level>`). Inside an interactive session `/model` accepts a free-text id verbatim, so a typed `model@preset/<slug>` keeps the provider policy while a bare model does not.
