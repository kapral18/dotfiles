...
Provenance enumerates every contributing config or registry source. Registry entries also name sections, such as `agent_bindings`, `agent_categories`, and `category_models` for Copilot review-policy routing.
...
`OPENROUTER_API_KEY` comes from the `openrouter/api/token` pass entry, exported by [`home/dot_config/fish/readonly_config.fish.tmpl`](../../../home/dot_config/fish/readonly_config.fish.tmpl). Both Pi and OpenCode resolve `openrouter/` selectors through their own built-in provider, so neither carries a generated provider block. `NVIDIA_API_KEY` comes from the `nvidia/nim/api/token` pass entry the same way; Pi and OMP both resolve `nvidia/` selectors through a built-in NVIDIA NIM provider (`https://integrate.api.nvidia.com/v1`), so no provider block backs those either.
...
- **Pi**: both profiles default to `openrouter/meta/muse-spark-1.3` with `defaultThinkingLevel: xhigh`. Category rows store model and effort separately: mechanical and memory use `openrouter/z-ai/glm-5.3-flash` at high, research uses `openrouter/z-ai/glm-5.3` at max and implement the same model at high, review uses `openrouter/meta/muse-spark-1.3` at max, and refute uses `openrouter/x-ai/grok-4.6` at high (cross-family counter to the Meta review lane). `pi_extra_models` includes those live catalog ids. Managed agent templates render `thinking:` from effort, so no `category_models.pi.*.model` contains a `:level` suffix. OpenRouter wrappers derive wire selectors only from the rows beginning with `openrouter/`; every Pi category row now qualifies, giving one wire per distinct model+effort (glm-flash high, glm max, glm high, muse max, grok high). GitHub Copilot rows never enter an OpenRouter allowlist, and `pi_extra_models` still lists several as selectable.

## Local inference

The local-inference backend is llama.cpp via `,llama-cpp`; see [llama.cpp local inference](llama-cpp/index.md). Pi can run its entire pricing there: `,pi-model-profile local` selects the `pi_model_profiles.local` rows (see [Pi model profiles](model-tiering.md#pi-model-profiles)).
