...
- **Pi**: `defaultProvider` is `anthropic` in both profiles, `defaultModel` is `claude-fable-5.1`, and `defaultThinkingLevel` is `high`, so the T1 lanes ride the native Anthropic route. `pi_extra_models` exposes the strict Pi OpenRouter set: `openrouter/openai/gpt-5.6-sol` (recommended OpenRouter route; T2 implement at `:high` and the refute counter at `:xhigh` — one id, two efforts, after it superseded `openrouter/openai/gpt-5.5` on 2026-09-07), `openrouter/z-ai/glm-5.3-flash` (mechanical since 2026-09-10), `openrouter/deepseek/deepseek-v4.1-flash` (V4 Flash carried mechanical until then; the id moved to V4.1 on 2026-09-11, still selectable), `openrouter/google/gemini-3.8-flash` (memory), `openrouter/anthropic/claude-sonnet-4.6` (former counter, still selectable), `openrouter/moonshotai/kimi-k3` (selectable), `openrouter/z-ai/glm-5.2` (selectable), and `openrouter/openai/gpt-6-astra` (selectable; pinned to OpenAI's Flex service tier through `only: ["openai/flex"]`, which bills $5/$25 per M against $10/$50 on the standard endpoint, live-probed 2026-09-11, user call; service-tier endpoints are never matched by the base `openai` slug). The gated Kimi, GLM 5.3 Flash, DeepSeek V4.1 Flash, GLM-5.2, and GPT-6 Astra provider policies still live in `modelOverrides`.
