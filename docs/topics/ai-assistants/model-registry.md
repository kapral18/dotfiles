...
- **Pi**: `defaultProvider` is `anthropic` in both profiles, `defaultModel` is `claude-fable-5.1`, and `defaultThinkingLevel` is `high`, so the T1 lanes ride the native Anthropic route. `pi_extra_models` exposes the strict Pi OpenRouter set: `openrouter/openai/gpt-5.6-sol` (recommended OpenRouter route; T2 implement at `:high` and the refute counter at `:xhigh` — one id, two efforts, after it superseded `openrouter/openai/gpt-5.5` on 2026-09-07), `openrouter/deepseek/deepseek-v4-flash` (mechanical), `openrouter/google/gemini-3.8-flash` (memory), `openrouter/anthropic/claude-sonnet-4.6` (former counter, still selectable), `openrouter/moonshotai/kimi-k3` (selectable), and `openrouter/z-ai/glm-5.2` (selectable). The gated Kimi, DeepSeek-0731, and GLM-5.2 provider policies still live in `modelOverrides`.
