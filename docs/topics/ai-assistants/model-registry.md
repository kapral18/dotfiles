...
- **Pi**: both profiles default to `github-copilot/claude-fable-5.1` with `defaultThinkingLevel: high`. Category rows store model and effort separately: research and review use that Fable route, mechanical and memory use `openrouter/z-ai/glm-5.3-flash` at high, implement uses `openrouter/meta/muse-spark-1.3` at high, and refute uses the same Meta model at max (cross-family counter to the Anthropic review lane). `pi_extra_models` includes those live catalog ids. Managed agent templates render `thinking:` from effort, so no `category_models.pi.*.model` contains a `:level` suffix. OpenRouter wrappers derive wire selectors only from the rows beginning with `openrouter/` (one wire per distinct model+effort: glm high, muse high, muse max); GitHub Copilot rows never enter an OpenRouter allowlist.
