...
- Legacy GH cache entries that predate the PR-author field bypass the full-scan TTL once. `live_refresh.sh` starts that full refresh immediately so PR-review row coloring repairs itself in the open picker instead of requiring a manual `alt-r`.
...
- On `ctrl-x` kill, plain folder/session hides are exact-path only: killing a parent folder session such as `~/code/repo` does not hide or kill child worktree entries such as `~/code/repo/main`. Worktree-backed selections still use subtree matching for sessions inside that worktree.
