...
| `,copilot`             | Launch GitHub Copilot CLI through one locked header-auth batch: deduplicate token sources, refresh each via `,mcp-token`, render/validate once, update the private config and `copilot-mcp` artifact row only on semantic change, then exec unchanged args; help/version/admin invocations bypass the batch                                                                                                                |
...
Large command internals live under `~/lib/,<command>/` while the public command stays in `~/bin`. The 28 library-backed commands are `,add-patch-to-prs`, `,ai`, `,ai-kb`, `,artifact`, `,codex`, `,codeowners`, `,copilot`, `,disable-auto-merge`, `,doctor`, `,enable-auto-merge`, `,get-age-buckets`, `,gh-prw`, `,gh-subissues-create`, `,gh-tfork`, `,gh-worktree`, `,hey-branch`, `,history-sync`, `,jest-test-title-report`, `,kbn-pr-audit`, `,kbn-stack`, `,llama-cpp`, `,mcp-token`, `,palantir`, `,proof`, `,pull-rebase`, `,tmux-run-all`, `,update`, `,w`, and `,wh`. Cross-command shell helpers live under `~/lib/shared/`.
