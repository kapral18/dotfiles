...
| `,copilot`             | Launch the real GitHub Copilot CLI; the stable entry point the `,copilot-*` BYOK wrappers exec through. Hosted MCP auth needs no launch-time work because those servers run as `,mcp-token --bridge` stdio bridges                                                                                                                                                                                                                       |
...
| `,mcp-token`           | Print a valid MCP token for `slack`/`scsi-main` from cursor's OAuth cache; direct `--login` guarantees runway (`--no-proactive-rotation` keeps proactive rotation off the critical path while the blocking window, expired tokens, and revoked tokens still rotate synchronously); `--bridge --url` serves a stdio MCP bridge that injects a fresh bearer per request, rotating behind the seam and resurrecting server-expired sessions |
