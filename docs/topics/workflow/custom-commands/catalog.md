...
| `,proof`               | Maintain a repo-external criteria/evidence/review ledger for ordinary freeform agent completion claims, with finalized seals, provenance visibility, secret scanning, and age-based pruning                                                                                                                                                                                                                                |
| `,palantir`            | Tmux-native detached orchestration: summon one legion per effort, supervise deterministic stage transitions, put criteria to machine trial, surface parked questions, and open the seeing-stone dashboard; source surfaces are `home/exact_bin/executable_,palantir`, `home/exact_lib/exact_,palantir/`, `home/dot_config/fish/completions/readonly_,palantir.fish`, and [Palantír docs](../../ai-assistants/palantir.md). |
...
| `,codex`               | Launch Codex with hosted-MCP bearer-token env vars captured, proactive rotation deferred until after capture, and local llama.cpp model catalog metadata injected when a local model is selected                                                                                                                                                                                                                           |
...
| `,copilot`             | Launch GitHub Copilot CLI through one locked header-auth batch: deduplicate token sources, capture each via `,mcp-token`, render/validate once, update the private config and `copilot-mcp` artifact row only on semantic change, start any due proactive rotations after capture, then exec unchanged args; help/version/admin invocations bypass the batch                                                               |
...
| `,cursor`              | Launch cursor-agent natively after validating each OAuth MCP token declared in `~/.cursor/mcp.json`; proactive rotation is deferred to Cursor's runtime, while critically short, expired, or revoked tokens still refresh synchronously; admin subcommands bypass the preflight                                                                                                                                            |
...
| `,mcp-token`           | Print a valid MCP token for `slack`/`scsi-main` from cursor's OAuth cache; direct `--login` guarantees runway, while `--launch-json` lets wrappers capture a still-valid token and defer proactive rotation off their critical path; the final blocking window, expired tokens, and revoked tokens still rotate synchronously, and `--rotate` performs deferred silent rotation without a browser fallback                 |
...
Plain interactive `claude` stays native. Plain interactive `codex`, `copilot`, and `cursor-agent` route through the managed `,codex` / `,copilot` / `,cursor` wrappers so hosted MCP tokens are validated and captured before launch; proactive rotation runs after capture where the consumer needs a fixed bearer (the `agent` alias also routes through `,cursor`). Local/provider-specific wrappers (`*,llama-cpp`, `,codex-cloudflare`, `,copilot-cloudflare`, `,copilot-litellm`, `,copilot-openrouter`) select their own upstreams.
