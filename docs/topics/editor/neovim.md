...
- `:PackDashboard` -> compact floating plugin dashboard that opens from cached/known state by default, with:
  ...
  - breaking-risk hint (best-effort) from semver delta (`major`/`minor`/`patch`) plus commit-message signals in the cumulative `rev_before..rev_after` range (for example `BREAKING`, `break:`, `break(scope):`, `feat`, `fix`, `refactor`, `perf`); `RK` shows `!` (breaking), `+` (safe), or `-` (unknown) with warn/green/muted coloring on update rows; the details popup (`K`) highlights commits with breaking signals in orange-yellow
