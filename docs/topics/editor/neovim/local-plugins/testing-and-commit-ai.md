...
| ---------- | -------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| OpenRouter | `OPENROUTER_API_KEY` | None. The route is fixed to `openai/gpt-oss-120b` with `reasoning.effort: "high"` (catalog max: high/medium/low), no `provider.sort` (uptime-aware default LB), `preferred_min_throughput: 300`, and `max_tokens: 131072` (endpoint max completion / 131072-token context). OpenRouter deprioritizes slower endpoints rather than hard-excluding them. Independent of the DeepSeek wrapper/Pi/OMP route. |
| Gemini     | `GEMINI_API_KEY`     | `GEMINI_MODEL` (default `gemini-flash-latest`, 1,048,576-token input limit), `GEMINI_MAX_OUTPUT_TOKENS` (default `65536`, Gemini 3.6 Flash output cap)                                                                                                                                                                                                                                                   |

The plugin sends the full staged diff. OpenRouter requests always enable the `context-compression` plugin, so prompts that exceed the 131072-token window are compressed instead of failing at the model context boundary.
