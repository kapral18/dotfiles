...
- When you type a query, the picker jumps the cursor back to the first match (so filtering always starts from the top, even if you previously scrolled far down).
...
- `ctrl-x` kills selected tmux sessions. `alt-x` removes selected worktrees:
  - For normal worktree selections: non-blocking; runs `,w remove` in the background.
    ...
    - The picker uses `,w remove --paths ...`, which also supports removing detached worktrees when explicitly selected.
